...
function easeLinear(t){ return Math.max(0,Math.min(1,t)); }
function currentReward(row, now = performance.now()) {
  // Animazione solo TRA due valori gia' verificati on-chain.
  // Non estrapola mai oltre il target confermato e quindi non crea reward futuri.
  if(!row.synced) return 0;
  const from=Math.max(0,number(row.displayFrom));
  const to=Math.max(0,number(row.displayTo));
  const started=number(row.displayStartedAt);
  const duration=Math.max(120,number(row.displayDuration)||1);
  if(!started || duration<=0) return to;
  const t=easeLinear((now-started)/duration);
  const value=from+(to-from)*t;
  row.displayedReward = t>=1 ? to : value;
  return Math.max(0,row.displayedReward);
}
...
function renderFrame(frameNow = 0) {
  ...
  if (frameNow - lastFramePaint < 60) return;
  lastFramePaint = frameNow;
  let total=0;
  state.rows.forEach((row)=>{
    const value=currentReward(row,frameNow);total+=value;
...
async function syncRewardFlow(force=false){
  ...
  try{
    ...
    settled.forEach((entry)=>{
      ...
      // Il rate serve solo alla card "produzione stimata"; il reward mostrato usa soltanto target on-chain verificati.
      ...
      // Il target visuale e' SEMPRE un valore gia' confermato on-chain.
      // In crescita, distribuiamo l'incremento verificato lungo quasi tutto il prossimo ciclo:
      // le cifre scorrono senza mai superare il dato confermato.
      // In caso di claim/prelievo, niente coda grafica: riallineamento immediato.
      const paintNow=performance.now();
      const visibleNow=row.synced ? currentReward(row,paintNow) : networkReward;
      row.confirmedReward=networkReward;
      if(!row.synced || claimDetected){
        row.displayedReward=networkReward;
        row.displayFrom=networkReward;
        row.displayTo=networkReward;
        row.displayStartedAt=paintNow;
        row.displayDuration=1;
      }else if(networkReward>=visibleNow){
        row.displayedReward=visibleNow;
        row.displayFrom=visibleNow;
        row.displayTo=networkReward;
        row.displayStartedAt=paintNow;
        row.displayDuration=Math.max(900,REWARD_REFRESH_MS-350);
      }else{
        // Una piccola correzione verso il basso non deve restare sullo schermo.
        row.displayedReward=networkReward;
        row.displayFrom=networkReward;
        row.displayTo=networkReward;
        row.displayStartedAt=paintNow;
