...
function money(value) {
  return smartMoney(value,state.currency==='EUR'?'EUR':'USD');
}
function fiatPairFromInj(inj){
  if(!(state.price>0))return '—';
  const usd=Math.max(0,number(inj))*state.price;
  const eur=usd*Math.max(0,number(state.eurRate)||0);
  return `${smartMoney(usd,'USD')} · ${smartMoney(eur,'EUR')}`;
}
...
function renderRows() {
  ...
  state.rows.forEach((row,index)=>{
    ...
    const apr=node.querySelector('[data-apr]');if(apr)apr.textContent=state.apr>0?`${state.apr.toFixed(3)}%`:'—';
...
function renderFrame(frameNow = 0) {
  ...
  state.rows.forEach((row)=>{
    ...
    if(Number.isFinite(value)){
      ...
      if(fiat) fiat.textContent=fiatPairFromInj(value);
    }else{
  ...
  if($('rewardsTotalFiat'))$('rewardsTotalFiat').textContent=syncedCount?fiatPairFromInj(total):'—';
...
async function syncStatus(force=false){
  ...
  try{
    ...
    settled.forEach((entry)=>{
      ...
      // Velocita' live deterministica: solo staking effettivo × APR rete.
      // Nessuna media con il saldo disponibile o con stime precedenti.
...
function boot(){
  ...
  // Prima otteniamo staking + APR, poi il reward verificato. In questo modo
