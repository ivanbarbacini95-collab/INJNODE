...
const state = {
  rows: MANAGED_ADDRESSES.map((address, index) => ({ address, index, label:`Wallet ${index + 1}`, staked:0, baseReward:0, baseAt:0, ratePerMs:0, online:false, synced:false })),
  apr: 0,
  price: 0,
  eurRate: 0.86,
  currency: 'USD',
...
async function loadWallet(address){const [delegations,rewards]=await Promise.all([lcd(`/cosmos/staking/v1beta1/delegations/${address}`),lcd(`/cosmos/distribution/v1beta1/delegators/${address}/rewards`)]);return{address,staked:delegationTotal(delegations),rewards:rewardTotal(rewards)}}

async function syncRewards(force=false){
  if(state.loading)return;if(!force&&state.lastSync&&Date.now()-state.lastSync<REFRESH_MS-1000)return;
  state.loading=true;$('rewardsLiveBadge')?.classList.add('sync');$('rewardsLiveBadge')?.classList.remove('error');
  const oldByAddress=new Map(state.rows.map((row)=>[row.address,{reward:row.baseReward,at:row.baseAt}]));
  try{
    const [apr,...settled]=await Promise.all([loadApr(),...MANAGED_ADDRESSES.map((address)=>loadWallet(address).then((value)=>({ok:true,value})).catch(()=>({ok:false,address}))) ]);
    if(apr>0)state.apr=apr;
    const now=Date.now();let liveCount=0;
    state.rows.forEach((row)=>{row.online=false;});
    settled.forEach((entry)=>{
      const address=entry.ok?entry.value.address:entry.address;const row=state.rows.find((item)=>item.address===address);if(!row)return;
      if(!entry.ok)return;
      const networkReward=Math.max(0,number(entry.value.rewards));const staked=Math.max(0,number(entry.value.staked));
      const prior=oldByAddress.get(address);let rate=staked*Math.max(0,state.apr)/100/YEAR_MS;
      if(prior?.at&&networkReward>=prior.reward){const elapsed=now-prior.at;const observed=elapsed>4000?(networkReward-prior.reward)/elapsed:0;if(observed>0&&observed<rate*8)rate=(rate*.45)+(observed*.55);}
      row.staked=staked;row.baseReward=networkReward;row.baseAt=now;row.ratePerMs=Math.max(0,rate);row.online=true;row.synced=true;liveCount++;
    });
    ...
    $('rewardsLiveBadge')?.classList.remove('sync','error');renderRows();
    document.querySelectorAll('[data-reward]').forEach((node)=>{node.classList.remove('flash');void node.offsetWidth;node.classList.add('flash')});
  }catch(_){$('rewardsLiveBadge')?.classList.remove('sync');$('rewardsLiveBadge')?.classList.add('error');renderRows();if($('rewardsUpdated'))$('rewardsUpdated').textContent=copy('Rete Injective temporaneamente non disponibile','Injective network temporarily unavailable');}
...
function boot(){
  ...
  Promise.allSettled([loadPrice(),loadEurRate(),syncRewards(true)]);connectPriceSocket();
  setInterval(()=>{if(!document.hidden)void syncRewards(false)},REFRESH_MS);
  setInterval(()=>{if(!document.hidden&&(!state.socket||state.socket.readyState!==WebSocket.OPEN))void loadPrice()},30000);
  document.addEventListener('visibilitychange',()=>{if(!document.hidden){readLabels();renderRows();void syncRewards(true);connectPriceSocket()}});
