...
async function syncRewardFlow(force=false){
  ...
  try{
    ...
    settled.forEach((entry)=>{
      ...
      const claimDetected=wasSynced && networkReward + CLAIM_DROP_EPSILON < previousNetwork;

      if(!claimDetected && wasSynced && previousAt && networkReward>=previousNetwork){
        const elapsed=now-previousAt;
        if(elapsed>2500){
          const observed=(networkReward-previousNetwork)/elapsed;
          const model=Math.max(0,row.ratePerMs);
          if(observed>0){
            if(model<=0) row.ratePerMs=observed;
            else if(observed<model*8) row.ratePerMs=(model*.35)+(observed*.65);
          }
        }
      }

      // Sia in crescita sia dopo un claim, il dato on-chain diventa subito la nuova base reale.
