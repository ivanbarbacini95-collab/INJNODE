...
function ensureEntryPnlTimelineRow() {
  ...
  const openPrice = number(state.entrySpark.openPrice) || number(state.marketFrames?.d1?.open);
  if (!current) {
...
function paintEntryPnlTimelineScale() {
  ...
  const pct = visibleBaseWorth > 0 ? (visiblePnl / visibleBaseWorth) * 100 : 0;
  if (summary) {
    summary.textContent = `${visiblePnl > 0 ? '+' : visiblePnl < 0 ? '−' : ''}${money(Math.abs(visiblePnl), 2)} · ${pct > 0 ? '+' : pct < 0 ? '−' : ''}${Math.abs(pct).toFixed(2)}%`;
    summary.className = `${visiblePnl > 0 ? 'positive' : visiblePnl < 0 ? 'negative' : 'neutral'} private`;
  }
}
...
function enterLiveChartsFromGate() {
  ...
  window.location.assign('./live-charts.html?v=15.99.8');
}
...
function enterOrderBookFromGate() {
  ...
  window.location.assign('./order-book.html?v=15.99.8');
}
...
function enterCommandCenterFromGate() {
  ...
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  window.location.assign('./command-center.html?v=15.99.8');
}
...
const state = {
  ...
  entryPnlTimeline: { address: '', rows: [], renderKey: '', frame: 0, saveAt: 0, lastPaintAt: 0, followLatest: true, inspectDay: 0 },
...
function rewardProductionMilestone(dailyValue) {
  const daily = Math.max(0, number(dailyValue));
  const epsilon = 1e-10;
  const milestones = [0.01, 0.05, 0.1, 0.5, 1, 2, 5, 10, 25, 50, 100, 250, 500, 1000];
  const index = milestones.findIndex((value) => value > daily + epsilon);
  if (index >= 0) return { goal: milestones[index], base: index > 0 ? milestones[index - 1] : 0 };

  // Beyond the preset ladder, continue automatically with a 1 → 2 → 5 cadence.
  const magnitude = 10 ** Math.floor(Math.log10(Math.max(1, daily)));
  const ladder = [1, 2, 5, 10].map((step) => step * magnitude);
  const ladderIndex = ladder.findIndex((value) => value > daily + epsilon);
  if (ladderIndex >= 0) return { goal: ladder[ladderIndex], base: ladderIndex > 0 ? ladder[ladderIndex - 1] : magnitude / 2 };
  return { goal: magnitude * 20, base: magnitude * 10 };
}

function rewardProductionEtaDays(requiredStake, currentStake, aprPercent) {
  const required = Math.max(0, number(requiredStake));
  const principal = Math.max(0, number(currentStake));
  const dailyRate = Math.max(0, number(aprPercent)) / 100 / 365;
  if (!(required > principal)) return 0;
  if (!(principal > 0) || !(dailyRate > 0)) return Infinity;
  const days = Math.log(required / principal) / Math.log(1 + dailyRate);
  return Number.isFinite(days) && days >= 0 ? Math.ceil(days) : Infinity;
}

function rewardProductionEtaCopy(days) {
  if (!Number.isFinite(days)) return 'ETA non disponibile';
  if (days <= 0) return 'Raggiunto';
  const date = new Date(Date.now() + days * 86400000);
  let duration;
  if (days < 2) duration = '< 2 giorni';
  else if (days < 60) duration = `${days} giorni`;
  else if (days < 730) duration = `${(days / 30.4375).toLocaleString('it-IT', { maximumFractionDigits: days < 305 ? 1 : 0 })} mesi`;
  else duration = `${(days / 365.25).toLocaleString('it-IT', { maximumFractionDigits: 1 })} anni`;
  return `${duration} · ${date.toLocaleDateString('it-IT', { day: '2-digit', month: 'short', year: date.getFullYear() !== new Date().getFullYear() ? 'numeric' : undefined })}`;
}

function renderRewardProductionTarget(active, dailyReward) {
  const currentStake = Math.max(0, number(state.staked));
  const aprPercent = Math.max(0, number(state.personalApr));
  const aprRate = aprPercent / 100;

  // The ladder is expressed as reward production (INJ/day), but the bar itself
  // advances ONLY with delegated staking. When the current delegated stake is
  // enough to generate one level, the next level is selected automatically.
  const producedPerDay = active && aprRate > 0 ? currentStake * aprRate / 365 : Math.max(0, number(dailyReward));
  const { goal, base } = rewardProductionMilestone(producedPerDay);
  const requiredStake = active && aprRate > 0 ? goal * 365 / aprRate : 0;
  const baseStake = active && aprRate > 0 && base > 0 ? base * 365 / aprRate : 0;
  const missingStake = Math.max(0, requiredStake - currentStake);
  const stakeSpan = Math.max(1e-12, requiredStake - baseStake);
  const progress = active ? Math.max(0, Math.min(100, ((currentStake - baseStake) / stakeSpan) * 100)) : 0;
  const etaDays = active ? rewardProductionEtaDays(requiredStake, currentStake, aprPercent) : Infinity;
  const etaTextValue = active ? rewardProductionEtaCopy(etaDays) : '—';
  const goalText = goal.toLocaleString('it-IT', { minimumFractionDigits: goal < 0.1 ? 2 : 0, maximumFractionDigits: goal < 1 ? 4 : 2 });

  if ($('rewardAutoTargetLabel')) $('rewardAutoTargetLabel').textContent = active ? `${goalText} INJ / giorno` : '—';
  if ($('rewardAutoTargetEta')) $('rewardAutoTargetEta').textContent = active ? `ETA compound ${etaTextValue}` : '—';
  if ($('rewardAutoTargetCurrent')) $('rewardAutoTargetCurrent').textContent = active ? formatInj(currentStake, 4) : '—';
  if ($('rewardAutoTargetMissing')) $('rewardAutoTargetMissing').textContent = active ? formatInj(missingStake, 4) : '—';
  if ($('rewardAutoTargetStake')) $('rewardAutoTargetStake').textContent = active ? formatInj(requiredStake, 4) : '—';
  ...
  if (panel) {
    ...
    panel.setAttribute('aria-label', active
      ? `Staking attuale ${currentStake.toFixed(4)} INJ. Prossimo livello ${goalText} INJ al giorno. Servono ${requiredStake.toFixed(4)} INJ in staking, ne mancano ${missingStake.toFixed(4)}. Tempo stimato con compound giornaliero ${etaTextValue}.`
      : 'Percorso staking non disponibile');
  }
}
function renderRewardTracker() {
  ...
  renderRewardProductionTarget(active, daily);
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.99.8').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
