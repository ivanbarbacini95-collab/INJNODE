...
function prepareImmersiveHomeCrossfade() {
  ...
  // Home has stayed rendered underneath Live/Pulse. Refresh only its live
  // overview, promote it above the immersive page at opacity 0, then fade it
  // in. The outgoing fullscreen page remains opaque below for every frame.
  renderEntryOverview();
  loadEntrySparkline(false);
  gate.classList.remove('leaving', 'mode-return', 'return-front', 'immersive-underlay', 'return-crossfade-visible');
  gate.classList.add('visible', 'return-crossfade');
  gate.setAttribute('aria-hidden', 'false');
  document.body.classList.add('entry-gate-open', 'mode-returning');
  document.body.classList.remove('entry-live-launch');
  void gate.offsetWidth;
  requestAnimationFrame(() => requestAnimationFrame(() => gate.classList.add('return-crossfade-visible')));
  return gate;
}
...
function closeFocusDisplay({ revealDashboard = true, animate = false } = {}) {
  ...
  if (!revealDashboard) {
    ...
    window.setTimeout(() => finishImmersiveHomeCrossfade(dialog, 'focus-display-open'), 460);
    return;
  }
...
function returnDashboardToEntryGate() {
  ...
  gate.classList.add('visible', 'dashboard-underlay');
  gate.setAttribute('aria-hidden', 'false');
...
function closePulseView({ revealDashboard = true, animate = false } = {}) {
  ...
  if (!revealDashboard) {
    ...
    window.setTimeout(() => finishImmersiveHomeCrossfade(overlay, 'pulse-view-open'), 460);
    return;
  }
...
function renderRewardTracker() {
  ...
  if ($('rewardUsdTargetTick')) $('rewardUsdTargetTick').textContent = money(rewardUsdTarget, rewardUsdTarget < 10 ? 2 : 0);
