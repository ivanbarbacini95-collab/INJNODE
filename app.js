...
// v15.99.55 — Mobile Home edge clamp.
// iOS/WebKit can rubber-band a fixed overflow scroller past its real bottom
// and then visibly snap it back. Clamp only outward vertical gestures at a
// scroll boundary; horizontal wallet/chart gestures remain untouched.
function installHomeMobileScrollGuard() {
  const gate = $('entryGate');
  if (!gate || gate.dataset.mobileEdgeGuard === '1') return;
  gate.dataset.mobileEdgeGuard = '1';
  ...
  const clampScroller = (scroller) => {
    const max = Math.max(0, scroller.scrollHeight - scroller.clientHeight);
    if (scroller.scrollTop < 0) scroller.scrollTop = 0;
    else if (scroller.scrollTop > max) scroller.scrollTop = max;
    return max;
  };

  gate.addEventListener('touchstart', (event) => {
    if (!mobileLike() || event.touches.length !== 1 || !gate.classList.contains('visible')) {
      tracking = false;
      return;
    }
  ...
  gate.addEventListener('touchmove', (event) => {
    ...
    // Preserve horizontal carousel/chart interaction.
    if (Math.abs(totalX) > Math.abs(totalY)) return;
    if (Math.abs(stepY) < 0.25) return;

    const scroller = verticalScrollerFor(event.target);
    const max = clampScroller(scroller);
    const atTop = scroller.scrollTop <= 0.5;
    const atBottom = scroller.scrollTop >= max - 0.5;
    const pullingPastTop = atTop && stepY > 0;
    const pullingPastBottom = atBottom && stepY < 0;

    if ((pullingPastTop || pullingPastBottom) && event.cancelable) {
      event.preventDefault();
    }
  }, { passive: false });
  ...
  gate.addEventListener('scroll', () => {
    if (!mobileLike() || !gate.classList.contains('visible')) return;
    clampScroller(gate);
  }, { passive: true });
}
