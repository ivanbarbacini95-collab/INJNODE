...
function renderTreasurySparkline() {
  ...
  const points = samples.map((row) => {
    const x = ((number(row.t) - minT) / timeSpan) * 320;
    const y = 79 - ((number(row.value) - min) / span) * 61;
    return [x, y];
  });
  const d = points.map(([x,y],index) => `${index ? 'L' : 'M'}${x.toFixed(2)} ${y.toFixed(2)}`).join(' ');
  const [lastX,lastY] = points[points.length - 1];
  const area = `${d} L${lastX.toFixed(2)} 92 L0 92 Z`;
  svg.innerHTML = `<path class="treasury-growth-area" d="${area}"></path><path class="treasury-growth-line" d="${d}"></path><circle cx="${lastX.toFixed(2)}" cy="${lastY.toFixed(2)}" r="3"></circle>`;
...
function renderTreasury() {
  ...
  setValue('treasuryTotalInj', total.toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 4 }), total, { roll: true });
...
const state = {
  ...
  treasury: { data: null, loading: false, updatedAt: 0, samples: [], historyLoaded: false, cacheHydrated: false, timer: 0 },
  endpoint: '',
  socket: null,
  walletRequest: 0,
