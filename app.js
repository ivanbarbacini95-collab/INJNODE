...
function updateEntryPnlTimelineTodayHero(current = null) {
  ...
  const row = current || state.entryPnlTimeline.rows?.find((item) => item.live) || null;
  if (summary) {
    if (!row) {
      summary.textContent = '—';
  ...
  if (rollover) {
    ...
    rollover.textContent = `Chiude alle 00:00 · ${String(hours).padStart(2, '0')}h ${String(minutes).padStart(2, '0')}m`;
  }
}
...
function paintEntryPnlTimelineScale() {
  ...
  candles.forEach((node) => {
    ...
    // Square-root easing preserves tiny candles when one very large day is
    // visible, while still making the dominant move obviously larger.
    const eased = Math.sqrt(ratio);
    const height = value === 0 ? 2 : Math.max(3, eased * maxVisual);
...
function renderEntryPnlTimeline(force = false) {
  ...
  if (force || renderKey !== timeline.renderKey) {
    ...
    const slot = count <= 7
      ? Math.max(46, Math.floor(scroll.clientWidth / Math.max(1, count)))
      : Math.max(66, Math.min(86, Math.round(scroll.clientWidth / 7.2) || 74));
    track.style.setProperty('--pnl-slot', `${slot}px`);
    track.style.width = count <= 7 ? '100%' : `${Math.max(scroll.clientWidth, count * slot)}px`;
    ...
    rows.forEach((row, index) => {
      ...
      day.innerHTML = `<span class="entry-pnl-value private">${row.pnlUsd > 0 ? '+' : row.pnlUsd < 0 ? '−' : ''}${money(Math.abs(row.pnlUsd), 2)}</span><i class="entry-pnl-candle"><b></b></i><small>${row.live ? 'OGGI' : entryPnlTimelineDayLabel(row.dayStart, true)}</small>`;
...
function updateEntryPnlTimelineLive(forcePaint = false) {
  ...
  const now = performance.now();
  if (!forcePaint && now - number(state.entryPnlTimeline.lastPaintAt) < 500) return;
  ...
  if (value) value.textContent = `${current.pnlUsd > 0 ? '+' : current.pnlUsd < 0 ? '−' : ''}${money(Math.abs(current.pnlUsd), 2)}`;
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.99.22').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
