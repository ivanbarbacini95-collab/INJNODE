...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.24').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function applyTheme(theme, notify = false) {
  const themes = {
    navy: { label: 'Aurora', color: '#03070b', base: 'navy' },
    black: { label: 'Obsidian', color: '#030303', base: 'black' },
    light: { label: 'Chiaro', color: '#eef5ff', base: 'light' },
    ice: { label: 'Ice Glass', color: '#081723', base: 'navy' }
  };
  const selected = themes[theme] ? theme : 'navy';
  const baseTheme = themes[selected].base;
  // Ice Glass is intentionally Dashboard-only: Home, Live View and Pulse View
  // continue to inherit Aurora while the Dashboard receives a dedicated class.
  document.documentElement.dataset.theme = baseTheme;
  document.documentElement.dataset.dashboardTheme = selected;
  document.body.classList.toggle('dashboard-theme-ice', selected === 'ice');
...
function bindMobileFastTap() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (button !== mobileFastTap.lastButton) return;
    if (performance.now() - mobileFastTap.lastAt > 750) return;
    event.preventDefault();
...
async function init() {
  ...
  applyTheme(document.documentElement.dataset.dashboardTheme || document.documentElement.dataset.theme);
