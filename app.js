...
function buildFocusLiveChartPath(samples, now = Date.now()) {
  ...
  const coords = visible.map((point) => {
    ...
    const y = Math.max(8, Math.min(252, 252 - ((visualPrice - min) / span) * 244));
    return { x, y };
  });
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.35').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
