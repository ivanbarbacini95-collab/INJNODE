...
function entrySparkPath(points) {
  ...
  return { line, area, direction: prices.at(-1) - open, last: coords.at(-1) };
}
...
function renderEntryAtGlance() {
  ...
  const dot = $('entrySparkDot');
  if (path) path.setAttribute('d', spark.line);
  if (area) area.setAttribute('d', spark.area);
  if (dot) {
    if (spark.last) {
      dot.setAttribute('cx', spark.last[0].toFixed(2));
      dot.setAttribute('cy', spark.last[1].toFixed(2));
      dot.style.opacity = '1';
    } else {
      dot.style.opacity = '0';
    }
...
const state = {
  ...
  entrySpark: { points: [], loadedAt: 0, loading: false, dayStart: 0, openPrice: 0, lastLiveSampleAt: 0 },
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.78').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
// v15.78 — Immersive desktop Home: viewport-filling responsive grid for fullscreen/large monitors.

// v15.78 — edge-to-edge mobile Home/Control Center; no outer shell frame or transformed fixed-menu containing block.
