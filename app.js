// v15.98.58 — Global Market Motion: Command Center-style realtime INJ price interpolation across Home, Dashboard, Live View and Pulse View.
// Price-derived values follow the same visual price without forcing full-page redraws.
// v15.98.58 — Command Center: isolated large-screen wall display with portfolio, staking, Pulse and five live charts.
...
function enterLiveChartsFromGate() {
  ...
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  window.location.assign('./live-charts.html?v=15.98.58');
}
...
function enterCommandCenterFromGate() {
  ...
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  window.location.assign('./command-center.html?v=15.98.58');
}
...
// v15.98.58 — one visual market clock shared by every main-app surface.
...
function connectMarketSocket() {
  ...
  try {
    // v15.98.58 — same lightweight Binance miniTicker feed used by Command Center.
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.58').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
// v15.98.58 — resume the shared visual price clock after iOS/browser visibility suspension.
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
