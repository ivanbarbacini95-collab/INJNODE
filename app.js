// v15.98.12 — Stable per-digit odometer on Home + Dashboard + Live View; Reward Tracker/Pulse use color/blink feedback only.
'use strict';
...
function setEntryAnimatedValue(id, text, numericValue, { directional = false } = {}) {
  ...
  // Home uses the same per-digit odometer as Dashboard. No whole-value lift:
  // unchanged digits, separators and symbols remain perfectly stationary.
...
function setPulseOnlyValue(id, text, numericValue) {
  const element = $(id);
  if (!element) return;
  const result = setPlainNumberValue(element, text, numericValue);
  if (!state.suppressEffects && result.visibleChange && result.numericChange) {
    signalChange(element, result.previous, result.next);
  }
}

// Pulse View intentionally has no numeric odometer in v15.98.12.

function setValue(id, text, numericValue, { flash = true } = {}) {
  const element = $(id);
  if (!element) return;

  const result = setRollingNumberValue(element, text, numericValue);
...
function renderMarketFrame(key, prefix) {
  ...
  if (!(open > 0) || !(current > 0) || !(min > 0) || !(max > 0)) {
    minEl.textContent = '—';
    setRollingNumberValue(openEl, open > 0 ? money(open, open < 1 ? 4 : 3) : '—', open > 0 ? open : NaN);
    maxEl.textContent = '—';
  ...
  setRollingNumberValue(minEl, money(min, digits(min)), min);
  setRollingNumberValue(openEl, money(open, digits(open)), open);
  setRollingNumberValue(maxEl, money(max, digits(max)), max);
  setRollingNumberValue(changeEl, `${changePercent > 0 ? '+' : ''}${changePercent.toFixed(Math.abs(changePercent) >= 100 ? 1 : 2)}%`, changePercent);
...
function setFocusDisplayValue(element, text, numericValue) {
  ...
  // v15.98.12: Live View keeps the per-digit odometer. While Live View is
  // hidden we only sync the final text, so background renders cannot start
  // invisible rolls or leave an animation half-way through when it is opened.
  if (active) {
    animateVerticalNumberRoll(element, previousText, nextText, previous, next);
  } else {
    clearTimeout(element._verticalRollTimer);
...
function renderMarket() {
  ...
  const next = Number(state.change);
  setRollingNumberValue(change, state.price > 0 ? `${next > 0 ? '+' : ''}${next.toFixed(2)}%` : '—', state.price > 0 ? next : NaN);
  ...
  setRollingNumberValue(amount, state.price > 0 ? usdt(state.changeAmount, 4) : '— USDT', state.price > 0 ? state.changeAmount : NaN);
  ...
  if (terminalChange) {
    setRollingNumberValue(terminalChange, state.price > 0 ? `${next > 0 ? '+' : ''}${next.toFixed(2)}% · 24H` : '—', state.price > 0 ? next : NaN);
...
function renderPortfolio() {
  ...
  if (state.address && state.price > 0) {
    ...
    setRollingNumberValue(worthChange, `${label} ${signedMoney(netWorthChange24h, 2)} (${state.change > 0 ? '+' : ''}${state.change.toFixed(2)}%)`, netWorthChange24h);
  } else {
  ...
  if (state.address && state.price > 0 && averageBuyPrice > 0) {
    ...
    setRollingNumberValue(pnlElement, `PnL totale ${prefix}${money(Math.abs(pnl), 2)} (${pnlPercent > 0 ? '+' : ''}${pnlPercent.toFixed(2)}%)`, pnl);
  } else {
...
function renderRewardTracker() {
  ...
  setPulseOnlyValue('hourlyEstimate', active ? formatInj(hourly, 4) : '—', active ? hourly : NaN);
  setPulseOnlyValue('hourlyUsd', active && valuationPrice > 0 ? money(hourly * valuationPrice, 4) : '—', active && valuationPrice > 0 ? hourly * valuationPrice : NaN);
  setPulseOnlyValue('dailyEstimate', active ? formatInj(daily, 4) : '—', active ? daily : NaN);
  setPulseOnlyValue('dailyUsd', active && valuationPrice > 0 ? money(daily * valuationPrice, 3) : '—', active && valuationPrice > 0 ? daily * valuationPrice : NaN);
  setPulseOnlyValue('weeklyEstimate', active ? formatInj(weekly, 4) : '—', active ? weekly : NaN);
  setPulseOnlyValue('weeklyUsd', active && valuationPrice > 0 ? money(weekly * valuationPrice, 3) : '—', active && valuationPrice > 0 ? weekly * valuationPrice : NaN);
  setPulseOnlyValue('monthlyEstimate', active ? formatInj(monthly, 4) : '—', active ? monthly : NaN);
  setPulseOnlyValue('monthlyUsd', active && valuationPrice > 0 ? money(monthly * valuationPrice) : '—', active && valuationPrice > 0 ? monthly * valuationPrice : NaN);
  setPulseOnlyValue('yearlyEstimate', active ? formatInj(yearly, 4) : '—', active ? yearly : NaN);
  setPulseOnlyValue('yearlyUsd', active && valuationPrice > 0 ? money(yearly * valuationPrice) : '—', active && valuationPrice > 0 ? yearly * valuationPrice : NaN);
  ...
  setPulseOnlyValue('rewardMilestoneLabel', active ? `${milestone.toLocaleString('it-IT')} INJ / giorno` : '—', active ? milestone : NaN);
  setPulseOnlyValue('rewardMilestoneMissing', active ? `${formatInj(missing, 4)} di staking mancanti` : '—', active ? missing : NaN);
...
function renderTarget() {
  ...
  setRollingNumberValue($('targetProgressCopy'), `${formatInj(total, 4)} / ${formatInj(target, 4)}`, total);
  $('targetProgressBar').style.width = `${percent}%`;
  setRollingNumberValue($('targetMissing'), formatInj(missing, 4), missing);
  setRollingNumberValue($('targetPercent'), `${percent.toFixed(2)}%`, percent);
  const targetDays = estimateTargetDays(target);
  setRollingNumberValue($('targetEta'), etaText(targetDays), Number.isFinite(targetDays) ? targetDays : NaN);
}
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.11').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
// v15.98.11 — Odometer scope: Home + Dashboard. Reward Tracker, Live View and Pulse View are blink/color-only.
