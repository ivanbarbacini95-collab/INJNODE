...
const state = {
  ...
  focusLiveChart: { samples: [], lastSampleAt: 0, lastTradeAt: 0, renderFrame: 0, lastRenderAt: 0, burstUntil: 0 },
...
const FOCUS_LIVE_MAX_SAMPLES = 720;
...
function updateFocusLiveTick(rawPrice, rawTime = Date.now()) {
  ...
  const minGap = MOBILE_EFFICIENCY ? 110 : 75;
  ...
  const cutoff = now - FOCUS_LIVE_WINDOW_MS - 2_000;
...
function focusLiveImpact(samples) {
  if (samples.length < 5) return { impact: 0, direction: 0 };
  const latest = samples.at(-1);
  let anchor = samples[0];
  for (let i = samples.length - 2; i >= 0; i--) {
    if (latest.t - samples[i].t >= 3000) { anchor = samples[i]; break; }
  }
  const steps = [];
  ...
  const typical = focusLiveMedian(steps) || latest.price * 0.000005;
  const delta = latest.price - anchor.price;
  const floor = Math.max(latest.price * 0.000025, typical * 5.5);
  const impact = Math.max(0, Math.min(1, Math.abs(delta) / Math.max(floor, 1e-10) - 0.12));
  return { impact, direction: Math.sign(delta) };
}
...
function focusLiveRenderSamples(samples, maxPoints = 150) {
...
function buildFocusLiveChartPath(samples, now = Date.now()) {
  if (!samples.length) return null;
  const windowStart = now - FOCUS_LIVE_WINDOW_MS;
  const visible = samples.filter((point) => point.t >= windowStart && point.price > 0);
  ...
  const minimumSpan = Math.max(latest * 0.000045, 0.00005);
  let span = max - min;
  if (span < minimumSpan) {
  ...
  const mid = (min + max) / 2;
  const compression = impact * 0.22;
  const renderVisible = focusLiveRenderSamples(visible);
  const coords = renderVisible.map((point) => {
    const age = Math.max(0, Math.min(1, (now - point.t) / FOCUS_LIVE_WINDOW_MS));
    // During a strong move, older deviations are gently compressed toward the
    // center. The live point remains unmodified and therefore owns the impulse.
    const historicalFactor = 1 - compression * Math.pow(age, 0.72);
    const visualPrice = mid + (point.price - mid) * historicalFactor;
    const x = Math.max(0, Math.min(1000, ((point.t - windowStart) / FOCUS_LIVE_WINDOW_MS) * 1000));
    const y = Math.max(18, Math.min(242, 242 - ((visualPrice - min) / span) * 224));
    return { x, y };
  });
...
function renderFocusLiveChart(force = false) {
  const host = $('focusLiveChart');
  const line = $('focusLiveChartLine');
  const area = $('focusLiveChartArea');
  const dot = $('focusLiveChartDot');
  if (!host || !line || !area || !dot) return;
  if (!force && !isFocusDisplayActive()) return;

  const chart = state.focusLiveChart;
  const now = Date.now();
  const path = buildFocusLiveChartPath(chart.samples, now);
  if (!path) {
    if (state.price > 0) updateFocusLiveTick(state.price, now);
    return;
  }

  line.setAttribute('d', path.line);
  area.setAttribute('d', path.area);
  dot.setAttribute('cx', path.last.x.toFixed(2));
  dot.setAttribute('cy', path.last.y.toFixed(2));

  const delta = chart.samples.length > 1 ? chart.samples.at(-1).price - chart.samples.at(-2).price : 0;
  // Use the short momentum direction first so the overlay stays elegant instead
  // of flashing color on every microscopic trade; fall back to the last tick.
  const direction = path.direction > 0 ? 'up' : path.direction < 0 ? 'down' : delta > 0 ? 'up' : delta < 0 ? 'down' : 'neutral';
  host.classList.remove('up', 'down', 'neutral', 'burst');
  host.classList.add(direction);
  if (path.impact >= 0.5) {
    host.classList.add('burst');
    chart.burstUntil = now + 900;
  } else if (chart.burstUntil > now) {
    host.classList.add('burst');
  }
}
...
function scheduleFocusLiveChartRender(force = false) {
  ...
  chart.renderFrame = requestAnimationFrame((timestamp) => {
    chart.renderFrame = 0;
    const minFrameGap = MOBILE_EFFICIENCY ? 80 : 50;
    if (!force && timestamp - chart.lastRenderAt < minFrameGap) {
      chart.renderFrame = requestAnimationFrame(() => {
        chart.renderFrame = 0;
        chart.lastRenderAt = performance.now();
        renderFocusLiveChart();
      });
      return;
    }
    chart.lastRenderAt = timestamp;
    renderFocusLiveChart(force);
  });
}
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.36').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
