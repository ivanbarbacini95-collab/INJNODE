...
function renderRewardLiquidityTarget(active, dailyReward) {
  ...
  const nearestWhole = Math.round(liquid);
  const isWhole = Math.abs(liquid - nearestWhole) < 1e-8;
  const goal = Math.max(1, isWhole ? nearestWhole + 1 : Math.ceil(liquid));
  const base = Math.max(0, goal - 1);
  const missing = Math.max(0, goal - liquid);
  const progress = Math.max(0, Math.min(100, (liquid - base) * 100));
  const eta = active ? rewardLiquidEta(missing, dailyReward) : '—';

  if ($('rewardLiquidTotal')) $('rewardLiquidTotal').textContent = state.address ? `${formatInj(liquid, 4)} INJ` : '—';
  if ($('rewardLiquidGoal')) $('rewardLiquidGoal').textContent = state.address ? `${goal.toLocaleString('it-IT')} INJ` : '—';
  if ($('rewardLiquidMissing')) $('rewardLiquidMissing').textContent = state.address ? `${formatInj(missing, 4)} INJ` : '—';
  ...
  if (panel) {
    ...
    panel.setAttribute('aria-label', state.address
      ? `Disponibile più reward ${formatInj(liquid, 4)} INJ. Mancano ${formatInj(missing, 4)} INJ per arrivare a ${goal.toLocaleString('it-IT')} INJ. Tempo stimato ${eta}.`
      : 'Prossimo INJ disponibile non disponibile');
  }
}
