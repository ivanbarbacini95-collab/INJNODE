...
// v15.98.2 — Compact standard K/M/B/T notation for Net Worth and wallet-value surfaces
...
function renderAggregate() {
  ...
  setValue('aggregateNetWorth', rows.length ? compactMoney(total.total * state.price) : '—', total.total * state.price, { flash: false });
...
function renderPulseView() {
  ...
  setPulseMetric(
    ...
    state.address && state.price > 0 ? compactMoney(total * state.price) : '—',
...
function renderPortfolio() {
  ...
  setValue('netWorthUsd', state.address ? compactMoney(netWorth) : '—', state.address ? netWorth : NaN, { flash: false });
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.2').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
