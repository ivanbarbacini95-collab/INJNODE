...
const LIVE_CHART_MAX_SAMPLES = 4200;
...
function updateLiveChartsTick(rawPrice, rawTime = Date.now()) {
  ...
  const minGap = MOBILE_EFFICIENCY ? 900 : 650;
...
function scheduleLiveChartsRender(force = false) {
  ...
  const frame = (timestamp) => {
    ...
    const indicatorGap = MOBILE_EFFICIENCY ? 34 : 24;   // smooth dot/marker
    const pathGap = MOBILE_EFFICIENCY ? 110 : 72;       // expensive 3-path rebuild
    const elapsed = timestamp - chart.lastRenderAt;
    if (forceThisFrame || elapsed >= indicatorGap) {
      chart.lastRenderAt = timestamp;
      const rebuildPaths = forceThisFrame || timestamp - number(chart.lastPathRenderAt) >= pathGap;
      if (rebuildPaths) chart.lastPathRenderAt = timestamp;
      renderLiveCharts(forceThisFrame, timestamp, rebuildPaths);
    }
...
function openLiveChartsView() {
  ...
  // Paint the destination first. History parsing/network merge and the first
  // three SVG builds happen only after the browser has displayed the screen.
  const token = ++chart.openToken;
  requestAnimationFrame(() => {
    if (token !== chart.openToken || !isLiveChartsActive()) return;
    if (state.price > 0 && !chart.samples.length) updateLiveChartsTick(state.price, Date.now());
