...
// v15.99.70 — Stable mobile notice + delayed IT/EN layer; core boot remains identical to v15.99.64.
...
function enterLiveChartsFromGate() {
  ...
  window.location.assign('./live-charts.html?v=15.99.16');
}
...
function enterOrderBookFromGate() {
  ...
  window.location.assign('./order-book.html?v=15.99.16');
}
...
function enterCommandCenterFromGate() {
  ...
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  window.location.assign(`./command-center.html?wallet=${encodeURIComponent(wallet.address)}&v=15.99.70`);
}
...
function commandCenterUnavailableNotice() {
  // v15.99.70 — Dedicated transient overlay. It is physically removed from
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.99.70').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function installHomeMobileScrollGuard() {
  ...
  gate.addEventListener('touchstart', (event) => {
    if (!mobileLike() || event.touches.length !== 1 || !gate.classList.contains('visible')) { tracking = false; return; }
