...
function enterCommandCenterFromGate() {
  // Command Centre is a dedicated desktop page. Keep this launch path independent
  // from the immersive-overlay gesture lock used by Dashboard/Live/Pulse.
  if (state.currentView !== 'home' || !commandCenterDesktopAllowed()) return;
  const wallet = activateEntrySelection();
  if (!wallet) return;
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  window.location.assign('./command-center.html?v=15.99.16');
}
