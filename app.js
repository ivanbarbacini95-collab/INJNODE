...
// v15.98.92 — Section-aware Dashboard navigation.
const DASHBOARD_SECTION_TARGETS = {
  overview: 'accountDetail',
  wallet: 'dashboardWalletSection',
  progress: 'walletGoalsCard',
  staking: 'dashboardStakingSection',
  market: 'marketMatrixSection',
  delegations: 'validatorCard'
};
let dashboardSectionNavFrame = 0;
let dashboardSectionNavReady = false;

function setDashboardSectionNavActive(key, keepVisible = true) {
  const nav = $('dashboardSectionNav');
  if (!nav) return;
  const buttons = [...nav.querySelectorAll('[data-dashboard-section]')];
  let activeButton = null;
  buttons.forEach((button) => {
    const active = button.dataset.dashboardSection === key;
    button.classList.toggle('active', active);
    button.setAttribute('aria-current', active ? 'true' : 'false');
    if (active) activeButton = button;
  });
  if (keepVisible && activeButton && window.innerWidth < 1280) {
    activeButton.scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
  }
}

function updateDashboardSectionNavigator(keepVisible = false) {
  dashboardSectionNavFrame = 0;
  if (state.currentView !== 'dashboard') return;
  const entries = Object.entries(DASHBOARD_SECTION_TARGETS)
    .map(([key, id]) => [key, $(id)])
    .filter(([, node]) => node && !node.hidden);
  if (!entries.length) return;

  const anchor = window.innerWidth < 1280 ? 92 : 120;
  ...
  entries.forEach(([key, node]) => {
    ...
    const top = rect.top - anchor;
    if (top <= 12 && top > bestPassedTop) {
      bestPassedTop = top;
      activeKey = key;
    } else if (bestPassedTop === -Infinity) {
  ...
  setDashboardSectionNavActive(activeKey, keepVisible);
}

function scheduleDashboardSectionNavigator() {
  if (dashboardSectionNavFrame) return;
  dashboardSectionNavFrame = requestAnimationFrame(() => updateDashboardSectionNavigator(false));
}

function initDashboardSectionNavigator() {
  const nav = $('dashboardSectionNav');
  if (!nav || dashboardSectionNavReady) return;
  dashboardSectionNavReady = true;
  nav.querySelectorAll('[data-dashboard-section]').forEach((button) => {
    button.addEventListener('click', () => {
      const key = button.dataset.dashboardSection;
      const target = $(DASHBOARD_SECTION_TARGETS[key]);
      if (!target) return;
      setDashboardSectionNavActive(key, true);
  ...
  window.addEventListener('scroll', scheduleDashboardSectionNavigator, { passive: true });
  window.addEventListener('resize', scheduleDashboardSectionNavigator, { passive: true });
}
