...
function renderDashboardHero() {
  ...
  setValue('dashboardHeroTotalInj', state.address ? `${formatInj(totalInj, 4)} totali` : '—', state.address ? totalInj : NaN, { flash: false });
  ...
  if (heroChange) {
    heroChange.textContent = state.address && state.price > 0
      ? `${netWorthChange24h > 0 ? '+' : netWorthChange24h < 0 ? '−' : ''}${money(Math.abs(netWorthChange24h), 2)} · ${change > 0 ? '+' : ''}${change.toFixed(2)}%`
      : '—';
...
function renderMarket() {
  ...
  if (change) {
    ...
    if (state.price > 0) change.dataset.numericValue = String(next);
  }
...
function renderRewardTracker() {
  ...
  if ($('rewardMilestoneBar')) $('rewardMilestoneBar').style.width = active ? `${Math.min(100, (daily / milestone) * 100)}%` : '0%';
  renderDashboardHero();
...
// v15.98.111 — Safe market rendering after the duplicate price nodes were removed from Dashboard.
