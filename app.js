// v15.98.8 — Vertical number roll for Dashboard, Live View and Pulse View.
'use strict';
...
function animateVerticalNumberRoll(element, previousText, nextText, previousNumeric, nextNumeric) {
  ...
  viewport.className = `number-roll-viewport number-roll-${direction}`;

  const oldLayer = document.createElement('span');
  oldLayer.className = 'number-roll-layer number-roll-old';
  oldLayer.textContent = oldText;

  const newLayer = document.createElement('span');
  newLayer.className = 'number-roll-layer number-roll-new';
  newLayer.textContent = newText;

  element.textContent = '';
  viewport.append(oldLayer, newLayer);
...
function setPulseExtremeUI(id, metricKey, row, fullScale, flash = '') {
  ...
  if (high) setRollingNumberValue(high, hasHigh ? pulseExtremeText(metricKey, row.ghostHigh) : '—', hasHigh ? Number(row.ghostHigh) : NaN);
  if (low) setRollingNumberValue(low, hasLow ? pulseExtremeText(metricKey, row.ghostLow) : '—', hasLow ? Number(row.ghostLow) : NaN);
...
function setPulseMetric(id, value, level, deltaText, currentText, metaText) {
  ...
  if (delta) setRollingNumberValue(delta, deltaText, n);
  if (current) {
    const currentNumeric = id === 'pulseMetricPrice' ? state.price
      : id === 'pulseMetricPnl' ? currentTotalInj() * state.price
      : state.rewards;
    setRollingNumberValue(current, currentText, currentNumeric);
  }
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.7').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
