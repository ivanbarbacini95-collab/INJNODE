...
function enterCommandCenterFromGate() {
  ...
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  window.location.assign('./command-center.html?v=15.99.2');
}
...
function renderRewardProductionTarget(active, dailyReward) {
  const daily = Math.max(0, number(dailyReward));
  const { goal, base } = rewardProductionMilestone(daily);
  const aprRate = Math.max(0, number(state.personalApr)) / 100;
  ...
  const span = Math.max(1e-12, goal - base);
  const progress = active ? Math.max(0, Math.min(100, ((daily - base) / span) * 100)) : 0;
  const etaDays = active ? rewardProductionEtaDays(requiredStake, currentStake, state.personalApr) : Infinity;
  const etaTextValue = active ? rewardProductionEtaCopy(etaDays) : '—';
  const goalText = goal.toLocaleString('it-IT', { maximumFractionDigits: goal < 1 ? 4 : 2 });

  if ($('rewardAutoTargetLabel')) $('rewardAutoTargetLabel').textContent = active ? `${goalText} INJ / giorno` : '—';
  if ($('rewardAutoTargetEta')) $('rewardAutoTargetEta').textContent = active ? `Tempo stimato ${etaTextValue}` : '—';
  if ($('rewardAutoTargetCurrent')) $('rewardAutoTargetCurrent').textContent = active ? `${daily.toLocaleString('it-IT', { minimumFractionDigits: 4, maximumFractionDigits: 4 })} INJ/g` : '—';
  if ($('rewardAutoTargetMissing')) $('rewardAutoTargetMissing').textContent = active ? `${formatInj(missingStake, 4)} staking` : '—';
  ...
  if (panel) {
    ...
    panel.setAttribute('aria-label', active
      ? `Produzione attuale ${daily.toFixed(4)} INJ al giorno. Prossimo target ${goalText} INJ al giorno. Mancano ${missingStake.toFixed(4)} INJ di staking. Tempo stimato ${etaTextValue}.`
      : 'Target produzione reward non disponibile');
  }
}
