...
function setEntryAnimatedValue(id, text, numericValue, { directional = false, roll = false, stableDigits = false } = {}) {
  ...
  if (roll) animateVerticalNumberRoll(element, previousText, nextText, previous, next);
  ...
  if (!state.suppressEffects && visibleChange && directional && numericChange) {
...
const state = {
  ...
  focusLiveChart: { samples: [], lastSampleAt: 0, lastTradeAt: 0, renderFrame: 0, lastRenderAt: 0, burstUntil: 0, visualPrice: 0, targetPrice: 0, animateUntil: 0, lastDirection: 0, lastScaleMin: 0, lastScaleMax: 0 },
...
function updateMarket(next) {
  ...
  if (nextPrice > 0) { state.price = nextPrice; if (Date.now() - number(state.focusLiveChart?.lastTradeAt) > 1400) updateFocusLiveTick(nextPrice, Date.now()); }
...
function connectMarketSocket() {
  ...
  try {
    // One combined Binance connection: ticker keeps the existing 24H metrics,
    // aggTrade feeds the ultra-sensitive 60-second Live View overlay.
    const socket = new WebSocket('wss://stream.binance.com:9443/stream?streams=injusdt@ticker/injusdt@aggTrade');
    state.socket = socket;
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (stream.endsWith('@aggTrade')) {
          updateFocusLiveTick(tick.p, tick.T || tick.E || Date.now());
          return;
        }
        updateFocusLiveTick(tick.c, tick.E || Date.now());
...
function setFocusDisplayValue(element, text, numericValue, { roll = false } = {}) {
  ...
  if (active && !state.suppressEffects && visibleChange && numericChange) {
    const direction = next > previous ? 'up' : 'down';
...
const FOCUS_LIVE_WINDOW_MS = 600_000;
const FOCUS_LIVE_MAX_SAMPLES = 1600;
const FOCUS_LIVE_RIGHT_X = 990;
const FOCUS_LIVE_LEFT_X = 12;

function updateFocusLiveTick(rawPrice, rawTime = Date.now()) {
  const price = number(rawPrice);
  if (!(price > 0)) return;
  const chart = state.focusLiveChart;
  const now = Number(rawTime) || Date.now();
  const minGap = MOBILE_EFFICIENCY ? 48 : 32;
  ...
  const cutoff = now - FOCUS_LIVE_WINDOW_MS - 20_000;
  while (chart.samples.length > 2 && chart.samples[0].t < cutoff) chart.samples.shift();
  if (chart.samples.length > FOCUS_LIVE_MAX_SAMPLES) chart.samples.splice(0, chart.samples.length - FOCUS_LIVE_MAX_SAMPLES);

  // Keep a short interpolation tail alive after every trade. The SVG endpoint
  // glides toward the real target instead of jumping between WebSocket ticks.
  chart.animateUntil = Math.max(chart.animateUntil || 0, performance.now() + (MOBILE_EFFICIENCY ? 220 : 180));
  if (isFocusDisplayActive()) scheduleFocusLiveChartRender();
}
...
function focusLiveImpact(samples) {
  if (samples.length < 4) return { impact: 0, direction: 0, threshold: 0 };
  const latest = samples.at(-1);
  let anchor = samples[0];
  for (let i = samples.length - 2; i >= 0; i--) {
    if (latest.t - samples[i].t >= 2600) { anchor = samples[i]; break; }
  }
  const steps = [];
  for (let i = Math.max(1, samples.length - 42); i < samples.length; i++) {
    const a = samples[i - 1].price;
    const b = samples[i].price;
    if (a > 0 && b > 0) steps.push(Math.abs(b - a));
  }
  const typical = focusLiveMedian(steps) || latest.price * 0.000004;
  const delta = latest.price - anchor.price;
  const threshold = Math.max(latest.price * 0.000012, typical * 1.9);
  const floor = Math.max(latest.price * 0.000025, typical * 5.2);
  const impact = Math.max(0, Math.min(1, Math.abs(delta) / Math.max(floor, 1e-10) - 0.10));
  return { impact, direction: Math.abs(delta) >= threshold ? Math.sign(delta) : 0, threshold };
}
...
function focusLiveRenderSamples(samples, maxPoints = 96) {
...
function focusLiveScalePrice(price) {
  return price > 0 ? money(price, price < 10 ? 4 : 3) : '—';
}

function buildFocusLiveChartPath(samples, visualPrice = 0, renderNow = Date.now()) {
  if (!samples.length && !(visualPrice > 0)) return null;
  const latestKnownPrice = visualPrice > 0 ? visualPrice : (samples.at(-1)?.price || 0);
  if (!(latestKnownPrice > 0)) return null;

  const windowEnd = Number(renderNow) || Date.now();
  const windowStart = windowEnd - FOCUS_LIVE_WINDOW_MS;
  const visible = [];
  let seedPrice = latestKnownPrice;

  for (let i = samples.length - 1; i >= 0; i--) {
    const point = samples[i];
    if (!(point?.price > 0)) continue;
    seedPrice = point.price;
    if (point.t < windowStart) break;
  }
  visible.push({ t: windowStart, price: seedPrice });
  for (const point of samples) {
    if (point.t < windowStart || !(point.price > 0)) continue;
    visible.push({ t: point.t, price: point.price });
  }

  const finalPrice = visualPrice > 0 ? visualPrice : (visible.at(-1)?.price || latestKnownPrice);
  if (visible.length && Math.abs((visible.at(-1)?.t || 0) - windowEnd) < 12) visible[visible.length - 1] = { t: windowEnd, price: finalPrice };
  else visible.push({ t: windowEnd, price: finalPrice });

  if (visible.length === 1) visible.unshift({ t: windowStart, price: finalPrice });

  const prices = visible.map((point) => point.price);
  let min = Math.min(...prices);
  let max = Math.max(...prices);
  const latest = visible.at(-1).price;
  const minimumSpan = Math.max(latest * 0.00016, 0.00008);
  let span = max - min;
  if (span < minimumSpan) {
  ...
  // Expand quickly on breakouts, relax more slowly for visual continuity.
  const chart = state.focusLiveChart || {};
  const nextMin = min - span * 0.14;
  const nextMax = max + span * 0.14;
  if (!(chart.lastScaleMin > 0) || !(chart.lastScaleMax > 0)) {
    chart.lastScaleMin = nextMin;
    chart.lastScaleMax = nextMax;
  } else {
    const expandAlpha = .46;
    const relaxAlpha = .08;
    chart.lastScaleMin += (nextMin - chart.lastScaleMin) * (nextMin < chart.lastScaleMin ? expandAlpha : relaxAlpha);
    chart.lastScaleMax += (nextMax - chart.lastScaleMax) * (nextMax > chart.lastScaleMax ? expandAlpha : relaxAlpha);
  }
  min = Math.min(chart.lastScaleMin, latest - minimumSpan * 0.52);
  max = Math.max(chart.lastScaleMax, latest + minimumSpan * 0.52);
  span = Math.max(max - min, minimumSpan);

  const { impact, direction } = focusLiveImpact(visible);
  const mid = (min + max) / 2;
  const compression = impact * 0.10;
  const renderVisible = focusLiveRenderSamples(visible, MOBILE_EFFICIENCY ? 120 : 150);
  const coords = renderVisible.map((point, index) => {
    const isLast = index === renderVisible.length - 1;
    const age = Math.max(0, Math.min(1, (windowEnd - point.t) / FOCUS_LIVE_WINDOW_MS));
    const historicalFactor = isLast ? 1 : 1 - compression * Math.pow(age, 0.8);
    const visual = mid + (point.price - mid) * historicalFactor;
    const ratio = Math.max(0, Math.min(1, (point.t - windowStart) / FOCUS_LIVE_WINDOW_MS));
    const x = isLast ? FOCUS_LIVE_RIGHT_X : FOCUS_LIVE_LEFT_X + ratio * (FOCUS_LIVE_RIGHT_X - FOCUS_LIVE_LEFT_X);
    const y = Math.max(18, Math.min(242, 242 - ((visual - min) / span) * 224));
    return { x, y };
  });
  ...
  const last = coords.at(-1);
  const area = `${line} L${last.x.toFixed(2)},260 L${first.x.toFixed(2)},260 Z`;
  return { line, area, last, impact, direction, min, max, mid, latest: finalPrice };
}

function renderFocusLiveChart(force = false, timestamp = performance.now()) {
  const host = $('focusLiveChart');
  const line = $('focusLiveChartLine');
  const area = $('focusLiveChartArea');
  const dot = $('focusLiveChartDot');
  if (!host || !line || !area || !dot) return false;
  if (!force && !isFocusDisplayActive()) return false;

  const chart = state.focusLiveChart;
  const target = chart.targetPrice > 0 ? chart.targetPrice : chart.samples.at(-1)?.price;
  if (target > 0) {
    if (!(chart.visualPrice > 0)) chart.visualPrice = target;
    const delta = target - chart.visualPrice;
    const epsilon = Math.max(target * 0.00000035, 0.000001);
    if (Math.abs(delta) <= epsilon) chart.visualPrice = target;
    else {
      // Time-independent easing: responsive enough for fast moves but never a
      // hard jump. A larger move naturally travels farther per frame.
      const frameGap = Math.max(8, Math.min(50, timestamp - (chart._visualFrameAt || timestamp - 16)));
      const alpha = 1 - Math.exp(-frameGap / (MOBILE_EFFICIENCY ? 72 : 60));
      chart.visualPrice += delta * alpha;
    }
    chart._visualFrameAt = timestamp;
  }

  const nowMs = Date.now();
  const path = buildFocusLiveChartPath(chart.samples, chart.visualPrice, nowMs);
  if (!path) {
    if (state.price > 0) updateFocusLiveTick(state.price, nowMs);
    return false;
  }

  line.setAttribute('d', path.line);
  area.setAttribute('d', path.area);
  dot.setAttribute('cx', String(FOCUS_LIVE_RIGHT_X));
  dot.setAttribute('cy', path.last.y.toFixed(2));

  const scaleTop = $('focusLiveChartScaleTop');
  const scaleMid = $('focusLiveChartScaleMid');
  const scaleBottom = $('focusLiveChartScaleBottom');
  const scaleLive = $('focusLiveChartScaleLive');
  const scaleMarker = $('focusLiveChartScaleMarker');
  if (scaleTop) scaleTop.textContent = focusLiveScalePrice(path.max);
  if (scaleMid) scaleMid.textContent = focusLiveScalePrice(path.mid);
  if (scaleBottom) scaleBottom.textContent = focusLiveScalePrice(path.min);
  if (scaleLive) scaleLive.textContent = focusLiveScalePrice(path.latest);
  if (scaleMarker) scaleMarker.style.top = `${(path.last.y / 260) * 100}%`;

  const rawLast = chart.samples.at(-1)?.price || chart.visualPrice;
  const previous = chart.samples.length > 1 ? chart.samples.at(-2).price : rawLast;
  const tickDelta = rawLast - previous;
  let directionValue = path.direction;
  if (!directionValue && Math.abs(tickDelta) > Math.max(rawLast * 0.000003, 0.000002)) directionValue = Math.sign(tickDelta);
  if (directionValue) chart.lastDirection = directionValue;
  else if (Date.now() - chart.lastTradeAt > 5000) chart.lastDirection = 0;

  const direction = chart.lastDirection > 0 ? 'up' : chart.lastDirection < 0 ? 'down' : 'neutral';
  host.classList.remove('up', 'down', 'neutral', 'burst');
  host.classList.add(direction);
  if (path.impact >= 0.5) {
    host.classList.add('burst');
    chart.burstUntil = Date.now() + 850;
  } else if (chart.burstUntil > Date.now()) {
    host.classList.add('burst');
  }

  return target > 0 && Math.abs(target - chart.visualPrice) > Math.max(target * 0.00000035, 0.000001);
}

function scheduleFocusLiveChartRender(force = false) {
  const chart = state.focusLiveChart;
  ...
  const frame = (timestamp) => {
    chart.renderFrame = 0;
    const forceThisFrame = force || chart._forceNext;
    chart._forceNext = false;
    const minFrameGap = MOBILE_EFFICIENCY ? 18 : 12;
    const elapsed = timestamp - chart.lastRenderAt;
    const shouldDraw = forceThisFrame || elapsed >= minFrameGap;
    let stillMoving = false;
    if (shouldDraw) {
      chart.lastRenderAt = timestamp;
      stillMoving = Boolean(renderFocusLiveChart(forceThisFrame, timestamp));
    }

    const keepAnimating = isFocusDisplayActive();
    if (keepAnimating) chart.renderFrame = requestAnimationFrame(frame);
  };

  chart.renderFrame = requestAnimationFrame(frame);
}
...
function openFocusDisplay() {
  ...
  if (state.price > 0 && !state.focusLiveChart.samples.length) updateFocusLiveTick(state.price, Date.now());
  renderFocusDisplay();
  renderFocusAmbient();
  dialog.hidden = false;
...
function renderPortfolio() {
  ...
  setValue('rewardsUsd', money(state.rewards * state.price), state.rewards * state.price);
...
function renderRewardTracker() {
  ...
  setPulseOnlyValue('hourlyEstimate', active ? formatInj(hourly, 4) : '—', active ? hourly : NaN, { flash: false });
  ...
  setPulseOnlyValue('dailyEstimate', active ? formatInj(daily, 4) : '—', active ? daily : NaN, { flash: false });
  ...
  setPulseOnlyValue('weeklyEstimate', active ? formatInj(weekly, 4) : '—', active ? weekly : NaN, { flash: false });
  ...
  setPulseOnlyValue('monthlyEstimate', active ? formatInj(monthly, 4) : '—', active ? monthly : NaN, { flash: false });
  ...
  setPulseOnlyValue('yearlyEstimate', active ? formatInj(yearly, 4) : '—', active ? yearly : NaN, { flash: false });
...
function renderTarget() {
  ...
  setPulseOnlyValue('targetProgressCopy', `${formatInj(total, 4)} / ${formatInj(target, 4)}`, total);
  $('targetProgressBar').style.width = `${percent}%`;
  setPulseOnlyValue('targetMissing', formatInj(missing, 4), missing);
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.38').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function bindEvents() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (!event.target?.closest?.('#entryDashboardButton, #entryLiveButton, #entryPulseButton')) return;
    event.preventDefault();
    event.stopImmediatePropagation();
  }, true);
  ...
  window.addEventListener('resize', () => { syncChartViewport(); syncFocusDisplayViewport(); scheduleFocusLiveChartRender(true); }, { passive: true });
  window.addEventListener('orientationchange', () => { requestAnimationFrame(() => { syncChartViewport(); syncFocusDisplayViewport(); scheduleFocusLiveChartRender(true); }); setTimeout(() => { syncChartViewport(); syncFocusDisplayViewport(); }, 180); }, { passive: true });
...
async function init() {
  ...
  renderFocusAmbient();
  setInterval(() => {
    if (state.address && !document.hidden && !state.loading && state.currentView !== 'home') loadWallet(false, state.address);
