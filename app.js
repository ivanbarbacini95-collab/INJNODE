...
function setEntryWalletSelection(address, { scroll = false, animatePreview = true } = {}) {
  ...
  if (changed && animatePreview) {
    const overview = $('entryOverview');
    if (overview) {
      const direction = previousIndex >= 0 && nextIndex < previousIndex ? 'prev' : 'next';
      overview.classList.remove('wallet-preview-shift', 'wallet-preview-next', 'wallet-preview-prev');
      void overview.offsetWidth;
      overview.classList.add('wallet-preview-shift', direction === 'prev' ? 'wallet-preview-prev' : 'wallet-preview-next');
      clearTimeout(entryWalletPreviewTimer);
      entryWalletPreviewTimer = window.setTimeout(() => overview.classList.remove('wallet-preview-shift', 'wallet-preview-next', 'wallet-preview-prev'), 320);
    }
  }
...
// v15.86 — Home title centered; wallet rail snaps to center and active-wallet content slides without transforming the painted card.
