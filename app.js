...
let entryWalletPreviewTimer = 0;
let entryWalletCardClone = null;

function cleanupEntryWalletCardSlide() {
  clearTimeout(entryWalletPreviewTimer);
  entryWalletPreviewTimer = 0;
  entryWalletCardClone?.remove();
  entryWalletCardClone = null;
  const overview = $('entryOverview');
  overview?.classList.remove('wallet-card-slide-in', 'wallet-card-slide-running', 'wallet-card-slide-next', 'wallet-card-slide-prev');
  overview?.style.removeProperty('--wallet-card-height');
  const shell = $('entryGate')?.querySelector('.entry-gate-shell');
  shell?.classList.remove('wallet-card-carousel-running');
}

function makeEntryWalletCardClone(overview) {
  const shell = $('entryGate')?.querySelector('.entry-gate-shell');
  if (!overview || !shell) return null;

  cleanupEntryWalletCardSlide();
  shell.classList.add('wallet-card-carousel-running');

  // Once the shell is positioned, offsets are stable and the outgoing card can
  // be parked exactly over the real one. It remains fully painted while the
  // replacement card is prepared, so iOS never exposes an empty/black frame.
  const left = overview.offsetLeft;
  const top = overview.offsetTop;
  const width = overview.offsetWidth;
  ...
  clone.classList.remove('wallet-preview-shift', 'wallet-preview-next', 'wallet-preview-prev');
  clone.classList.add('entry-overview-wallet-clone');
  ...
  entryWalletCardClone = clone;
  return clone;
}


/* v15.88 — finger-tracked wallet card rail + velocity flick navigation. */
...
function removeEntryWalletGestureClone() {
  entryWalletGesture.clone?.remove();
  entryWalletGesture.clone = null;
  entryWalletGesture.previewIndex = -1;
}
...
function resetEntryWalletGestureVisuals({ keepTracking = false } = {}) {
  ...
  if (overview) {
    overview.style.removeProperty('transition');
    overview.style.removeProperty('-webkit-transition');
    overview.style.removeProperty('transform');
    overview.style.removeProperty('-webkit-transform');
  }
  removeEntryWalletGestureClone();
  shell?.classList.remove('wallet-card-live-drag');
  host?.classList.remove('wallet-gesture-active');
...
function makeEntryWalletGestureClone(address) {
  const overview = $('entryOverview');
  const shell = $('entryGate')?.querySelector('.entry-gate-shell');
  if (!overview || !shell || !address) return null;
  ...
  clone.querySelectorAll('[aria-live]').forEach((node) => node.removeAttribute('aria-live'));
  clone.setAttribute('aria-hidden', 'true');
  clone.classList.remove('wallet-card-slide-in', 'wallet-card-slide-running', 'wallet-card-slide-next', 'wallet-card-slide-prev', 'wallet-preview-shift', 'wallet-preview-next', 'wallet-preview-prev');
  clone.classList.add('entry-overview-wallet-gesture-clone');
  clone.style.left = `${overview.offsetLeft}px`;
  clone.style.top = `${overview.offsetTop}px`;
  clone.style.width = `${overview.offsetWidth}px`;
  clone.style.height = `${overview.offsetHeight}px`;
  clone.style.minHeight = `${overview.offsetHeight}px`;
  shell.appendChild(clone);
  entryWalletGesture.clone = clone;
  return clone;
}
...
function ensureEntryWalletGestureTracking() {
  ...
  $('entryGate')?.querySelector('.entry-gate-shell')?.classList.add('wallet-card-live-drag');
}
...
function updateEntryWalletGesturePreview() {
  ...
  const overview = $('entryOverview');
  const shell = $('entryGate')?.querySelector('.entry-gate-shell');
  if (!host || !overview || !shell || state.wallets.length < 2) return;
  ...
  const previewIndex = Math.max(0, Math.min(rows.length - 1, baseIndex + direction));

  if (!direction || previewIndex === baseIndex) {
    overview.style.transform = 'translate3d(0,0,0)';
    overview.style.webkitTransform = 'translate3d(0,0,0)';
    removeEntryWalletGestureClone();
    return;
  }

  const previewRow = rows[previewIndex];
  const previewLeft = entryWalletCenteredScrollLeft(host, previewRow);
  const step = Math.max(1, Math.abs(previewLeft - baseLeft));
  const progress = Math.max(-1, Math.min(1, delta / step));

  if (entryWalletGesture.previewIndex !== previewIndex || !entryWalletGesture.clone) {
    entryWalletGesture.previewIndex = previewIndex;
    makeEntryWalletGestureClone(state.wallets[previewIndex]?.address);
  }
  const clone = entryWalletGesture.clone;
  if (!clone) return;

  // Current card follows the rail one-for-one. The neighbouring account card is
  // already painted offscreen and enters at the exact complementary distance.
  const currentPct = -progress * 100;
  const incomingPct = progress > 0 ? (1 - progress) * 100 : (-1 - progress) * 100;
  overview.style.transition = 'none';
  overview.style.webkitTransition = 'none';
  overview.style.transform = `translate3d(${currentPct.toFixed(3)}%,0,0)`;
  overview.style.webkitTransform = `translate3d(${currentPct.toFixed(3)}%,0,0)`;
  clone.style.transform = `translate3d(${incomingPct.toFixed(3)}%,0,0)`;
  clone.style.webkitTransform = `translate3d(${incomingPct.toFixed(3)}%,0,0)`;
}
...
function settleEntryWalletGesture(index, { behavior = 'smooth' } = {}) {
  ...
  clearTimeout(entryWalletGesture.settleTimer);
  entryWalletGesture.settleTimer = window.setTimeout(finalizeEntryWalletGesture, behavior === 'smooth' ? 280 : 40);
}

function finalizeEntryWalletGesture() {
  const host = $('entryWallets');
  const overview = $('entryOverview');
  if (!host || !overview) return resetEntryWalletGestureVisuals();
  const rows = [...host.querySelectorAll('.entry-wallet-row')];
  if (!rows.length) return resetEntryWalletGestureVisuals();

  let index = entryWalletGesture.targetIndex;
  if (!(index >= 0 && index < rows.length)) {
    const center = host.scrollLeft + host.clientWidth / 2;
    let best = 0;
    let distance = Infinity;
    rows.forEach((row, i) => {
      const d = Math.abs(row.offsetLeft + row.offsetWidth / 2 - center);
      if (d < distance) { distance = d; best = i; }
    });
    index = best;
  }

  const address = state.wallets[index]?.address;
  const cover = entryWalletGesture.clone;
  // Keep the incoming painted clone over the real card while the real card is
  // populated with the new wallet; remove it only on the next painted frame.
  overview.style.transition = 'none';
  overview.style.webkitTransition = 'none';
  overview.style.transform = 'translate3d(0,0,0)';
  overview.style.webkitTransform = 'translate3d(0,0,0)';
  if (address) setEntryWalletSelection(address, { scroll: false, animatePreview: false });
  const row = rows[index];
  if (row) host.scrollLeft = entryWalletCenteredScrollLeft(host, row);

  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      if (cover && cover === entryWalletGesture.clone) removeEntryWalletGestureClone();
...
function bindEntryWalletRailGestures(host) {
  ...
  const end = () => {
    ...
    entryWalletGesture.active = false;
    const hostNow = $('entryWallets');
    const rows = hostNow ? [...hostNow.querySelectorAll('.entry-wallet-row')] : [];
    const base = entryWalletGesture.baseIndex;
    if (!hostNow || !rows.length || base < 0) return resetEntryWalletGestureVisuals();

    const baseLeft = entryWalletCenteredScrollLeft(hostNow, rows[base]);
    const nextIndex = Math.min(rows.length - 1, base + 1);
    const prevIndex = Math.max(0, base - 1);
    const referenceRow = hostNow.scrollLeft >= baseLeft ? rows[nextIndex] : rows[prevIndex];
    const referenceLeft = entryWalletCenteredScrollLeft(hostNow, referenceRow);
    const step = Math.max(1, Math.abs(referenceLeft - baseLeft));
    const progress = (hostNow.scrollLeft - baseLeft) / step;
    ...
    let target = base;
    if (fastFlick) {
      // Finger launched left -> advance to the next wallet. Finger launched
      // right -> return to the previous wallet, exactly like a native carousel.
  ...
  host.addEventListener('touchstart', (event) => begin(event.touches?.[0]?.clientX), { passive: true });
  host.addEventListener('touchmove', (event) => move(event.touches?.[0]?.clientX), { passive: true });
  host.addEventListener('touchend', end, { passive: true });
  host.addEventListener('touchcancel', end, { passive: true });

  host.addEventListener('pointerdown', (event) => {
    if (event.pointerType === 'mouse') begin(event.clientX);
  }, { passive: true });
...
function startEntryWalletCardSlide(overview, clone, direction) {
  if (!overview || !clone) return;
  const dirClass = direction === 'prev' ? 'wallet-card-slide-prev' : 'wallet-card-slide-next';
  clone.classList.add('wallet-card-slide-out', dirClass);
  overview.classList.add('wallet-card-slide-in', dirClass);

  // Force one painted frame with both cards in their starting positions before
  // the GPU transition begins. This is important on mobile Safari/PWA.
  void clone.offsetWidth;
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      clone.classList.add('wallet-card-slide-running');
      overview.classList.add('wallet-card-slide-running');
    });
  });

  entryWalletPreviewTimer = window.setTimeout(cleanupEntryWalletCardSlide, 430);
}

function setEntryWalletSelection(address, { scroll = false, animatePreview = true } = {}) {
  if (!address || !state.wallets.some((wallet) => wallet.address === address)) return;
  const previousAddress = state.entrySelectedAddress;
  const changed = previousAddress !== address;
  const previousIndex = state.wallets.findIndex((wallet) => wallet.address === previousAddress);
  const nextIndex = state.wallets.findIndex((wallet) => wallet.address === address);
  const overview = $('entryOverview');
  const direction = previousIndex >= 0 && nextIndex < previousIndex ? 'prev' : 'next';
  const outgoingClone = changed && animatePreview && overview ? makeEntryWalletCardClone(overview) : null;
  ...
  // The real card is updated while the fully-painted outgoing clone still sits
  // on top of it. Then both cards move together in opposite directions.
  renderEntryOverview();
  if (outgoingClone && overview) startEntryWalletCardSlide(overview, outgoingClone, direction);
...
function syncEntryWalletSelectionFromRail() {
  const host = $('entryWallets');
  if (!host || state.wallets.length < 2) return;
  const rows = [...host.querySelectorAll('.entry-wallet-row')];
  if (!rows.length) return;
  ...
  if (nearest?.dataset.address && nearest.dataset.address !== state.entrySelectedAddress) {
    setEntryWalletSelection(nearest.dataset.address, { scroll: false, animatePreview: true });
  }
}
...
function renderEntryGate() {
  ...
  host.replaceChildren();
  hint.textContent = state.wallets.length === 1 ? '1 wallet salvato' : 'Scorri per cambiare wallet';
  ...
  state.wallets.forEach((wallet) => {
    ...
    button.addEventListener('click', () => {
      setEntryWalletSelection(wallet.address, { scroll: true, animatePreview: true });
    });
  ...
  // v15.88: the rail drives the active-wallet card continuously. A fast flick
  // advances exactly one wallet; a short/slow drag springs back unless the
  // neighbouring account crosses the commit threshold.
  host.onscroll = null;
...
// v15.88 — Wallet rail and active card are finger-tracked; velocity flick advances one account.
