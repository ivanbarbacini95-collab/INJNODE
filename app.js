...
function enterCommandCenterFromGate() {
  if (!modeLaunchAllowed() || !commandCenterDesktopAllowed()) return;
  const wallet = activateEntrySelection();
  if (!wallet) return;
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  window.location.assign('./command-center.html?v=15.99.5');
}
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.99.2').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
