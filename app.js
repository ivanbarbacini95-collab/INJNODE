// v15.98.31 — Global Card Elements: stronger visibility across Home, Dashboard, Live View and Pulse View.
// v15.98.30 — Card Elements: 10 lightweight persistent card-decoration presets compatible with every theme.
...
function bindEntrySparkInteraction() {
  ...
  stage.addEventListener('pointerdown', (event) => {
    ...
    // On touch, do not capture immediately: pan-y must remain available to the
    // page. Horizontal intent upgrades to chart inspection only after a small
  ...
  stage.addEventListener('pointercancel', () => { resetTouch(); hideEntrySparkInspector(); });
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.31').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function applyCardStyle(style, notify = false) {
  const selected = CARD_STYLE_META[style] ? style : 'orbit';
  document.documentElement.dataset.cardStyle = selected;
  localStorage.setItem('inj_monitor_card_style', selected);
  const label = $('cardStyleLabel');
  if (label) label.textContent = `${CARD_STYLE_META[selected]} · 10 stili`;
  $('cardStyleButton')?.setAttribute('aria-label', `Elementi card: ${CARD_STYLE_META[selected]}. Cambia stile`);
  document.querySelectorAll('[data-card-style-choice]').forEach((button) => {
    const active = button.dataset.cardStyleChoice === selected;
    button.classList.toggle('active', active);
    button.setAttribute('aria-pressed', active ? 'true' : 'false');
  });
  if (notify) toast(`Elementi card: ${CARD_STYLE_META[selected]}`);
}

function setCardStylePickerOpen(open) {
  $('cardStyleControl')?.classList.toggle('open', open);
  $('cardStyleButton')?.setAttribute('aria-expanded', open ? 'true' : 'false');
}
...
function bindMobileFastTap() {
  ...
  document.addEventListener('click', (event) => {
    ...
    const themeRetargetRisk = mobileFastTap.lastButton?.matches?.('[data-theme-choice]');
...
function bindEvents() {
  ...
  $('cardStyleButton')?.addEventListener('pointerdown', () => guardThemeInteraction(), { passive: true });
  $('cardStyleButton')?.addEventListener('click', (event) => {
    event.preventDefault();
    event.stopPropagation();
    guardThemeInteraction();
    const open = !$('cardStyleControl')?.classList.contains('open');
    if (open) {
      setSearchOpen(false);
      setThemePickerOpen(false);
    }
    setCardStylePickerOpen(open);
  });
  document.querySelectorAll('[data-card-style-choice]').forEach((button) => {
    button.addEventListener('pointerdown', () => guardThemeInteraction(650), { passive: true });
    button.addEventListener('click', (event) => {
      event.preventDefault();
      event.stopPropagation();
      guardThemeInteraction(650);
      applyCardStyle(button.dataset.cardStyleChoice, true);
      setCardStylePickerOpen(false);
    });
  });
  ...
  document.addEventListener('pointerdown', (event) => {
    ...
    if ($('cardStyleControl')?.classList.contains('open') && !$('cardStyleControl').contains(event.target)) setCardStylePickerOpen(false);
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'Escape' && $('cardStyleControl')?.classList.contains('open')) setCardStylePickerOpen(false);
...
async function init() {
  ...
  applyCardStyle(document.documentElement.dataset.cardStyle || localStorage.getItem('inj_monitor_card_style') || 'orbit');
...
// v15.98.30 — 10 selectable card-decoration presets persisted locally and theme-neutral.
// v15.98.31 — Preset surfaces expanded to Home, Dashboard, Live View and Pulse View.
