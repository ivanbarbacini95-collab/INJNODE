// v15.98.14 — Selective odometer: Home rolls only account countervalue + side INJ/USDT price.
...
function setEntryAnimatedValue(id, text, numericValue, { directional = false, roll = false } = {}) {
  const element = $(id);
  if (!element) return;
  ...
  // v15.98.14 — Home odometer is deliberately sparse. Only callers that pass
  // roll:true animate individual digits; all other Home values stay fixed.
...
function renderEntryOverview() {
  ...
  setEntryAnimatedValue('entryWalletWorth', total > 0 && state.price > 0 ? compactMoney(worth) : '—', worth, { roll: true });
...
function renderMarketFrame(key, prefix) {
  ...
  setBlinkOnlyElement(changeEl, `${changePercent > 0 ? '+' : ''}${changePercent.toFixed(Math.abs(changePercent) >= 100 ? 1 : 2)}%`, changePercent);
...
function setFocusDisplayValue(element, text, numericValue, { roll = false } = {}) {
  ...
  if (active && roll) {
    ...
    clearTimeout(element._verticalRollTimer);
    element._verticalRollTimer = null;
    if (roll) element.classList.add('number-roll-host');
    else element.classList.remove('number-roll-host');
    element.textContent = nextText;
  }
...
function renderMarket() {
  ...
  const next = Number(state.change);
  setBlinkOnlyElement(change, state.price > 0 ? `${next > 0 ? '+' : ''}${next.toFixed(2)}%` : '—', state.price > 0 ? next : NaN);
  ...
  if (terminalChange) {
    setBlinkOnlyElement(terminalChange, state.price > 0 ? `${next > 0 ? '+' : ''}${next.toFixed(2)}% · 24H` : '—', state.price > 0 ? next : NaN);
...
function renderPortfolio() {
  ...
  if (state.address && state.price > 0) {
    ...
    setBlinkOnlyElement(worthChange, `${label} ${signedMoney(netWorthChange24h, 2)} (${state.change > 0 ? '+' : ''}${state.change.toFixed(2)}%)`, netWorthChange24h);
  } else {
  ...
  if (state.address && state.price > 0 && averageBuyPrice > 0) {
    ...
    setBlinkOnlyElement(pnlElement, `PnL totale ${prefix}${money(Math.abs(pnl), 2)} (${pnlPercent > 0 ? '+' : ''}${pnlPercent.toFixed(2)}%)`, pnl);
  } else {
