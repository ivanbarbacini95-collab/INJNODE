...
function renderEntryGate() {
  ...
  commandCenterButton.disabled = !commandCenterDesktopAllowed();
...
function homeCommandCenterAllowed() {
  return window.matchMedia('(min-width: 1100px), (orientation: landscape) and (max-height: 560px)').matches;
}

function commandCenterDesktopAllowed() {
  return homeCommandCenterAllowed();
}

function syncHomeCommandCenter({ forceLoad = false } = {}) {
  const section = $('entryCommandCenterHome');
  const frame = $('entryCommandCenterFrame');
  if (!section || !frame) return;

  const wallet = selectedEntryWallet();
  const visible = state.currentView === 'home' && Boolean(wallet) && homeCommandCenterAllowed();
  const launcher = $('entryCommandCenterButton');
  if (launcher) launcher.disabled = !visible;

  section.hidden = !visible;
  section.setAttribute('aria-hidden', visible ? 'false' : 'true');
  document.body.classList.toggle('home-command-center-enabled', visible);
  ...
  const url = `./command-center.html?embedded=1&wallet=${encodeURIComponent(wallet.address)}&v=15.99.63`;
  if (forceLoad || frame.dataset.loadedUrl !== url) {
...
function enterCommandCenterFromGate() {
  // v15.99.63 — Command Centre is part of Home. The launcher is now only a
  // shortcut to the embedded panel: no page navigation and no Home reload.
  if (state.currentView !== 'home' || !homeCommandCenterAllowed()) return;
  const wallet = selectedEntryWallet();
  if (!wallet) return;
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  syncHomeCommandCenter({ forceLoad: true });
  requestAnimationFrame(() => {
    $('entryCommandCenterHome')?.scrollIntoView({
