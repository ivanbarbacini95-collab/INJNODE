...
const state = {
  ...
  treasuryPreview: { available: 0, staked: 0, rewards: 0, totalInj: 0, apr: 0, updatedAt: 0, loading: false, ok: false }
};
...
function treasuryPreviewMoney(usdValue) {
  return money(number(usdValue), 0);
}

function renderTreasury() {
  const t = state.treasuryPreview || {};
  const total = number(t.totalInj);
  const worth = total > 0 && state.price > 0 ? total * state.price : 0;
  const totalText = total > 0
    ? `${total.toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} INJ`
    : '— INJ';
  const worthText = worth > 0 ? treasuryPreviewMoney(worth) : (t.loading ? 'Sincronizzazione…' : 'LIVE');

  const totalNode = $('entryTreasuryTotal');
  const worthNode = $('entryTreasuryWorth');
  const publicTotal = $('entryTreasuryPublicTotal');
  const publicValue = $('entryTreasuryPublicValue');
  const menuHint = $('menuTreasuryHint');

  if (totalNode) totalNode.textContent = totalText;
  if (worthNode) worthNode.textContent = worthText;
  if (publicTotal) publicTotal.textContent = total > 0 ? totalText : 'Treasury pubblica';
  if (publicValue) publicValue.textContent = worth > 0 ? ` · ${worthText}` : ' · wallet nascosti';
  if (menuHint) menuHint.textContent = total > 0 ? `${totalText} · indirizzi protetti` : 'Aggregato pubblico · wallet nascosti';

  [$('entryTreasuryButton'), $('entryTreasuryPublicButton'), $('treasuryMenuButton')]
    .filter(Boolean)
    .forEach((el) => {
      el.classList.toggle('treasury-online', Boolean(t.ok && total > 0));
      el.classList.toggle('treasury-loading', Boolean(t.loading));
    });
}

async function loadTreasuryPreview(force = false) {
  const t = state.treasuryPreview;
  if (!t || t.loading) return;
  if (!force && t.updatedAt && Date.now() - number(t.updatedAt) < 10_000) return;
  ...
  try {
    ...
    if (!response.ok || !data?.ok || !data?.treasury) {
      throw new Error(data?.message || 'Treasury non disponibile');
    }
    const row = data.treasury;
    t.available = number(row.available);
    t.staked = number(row.staked);
    t.rewards = number(row.rewards);
    t.totalInj = number(row.totalInj);
    t.apr = number(row.apr);
    t.updatedAt = Date.parse(row.updatedAt || '') || Date.now();
    t.ok = true;
  } catch (_) {
    t.ok = false;
  } finally {
...
function enterTreasuryFromGate() {
  ...
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  try { sessionStorage.setItem('inj_node_treasury_from_home', '1'); } catch (_) {}
  window.location.assign('./treasury.html?v=15.99.25');
}
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.99.24').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function bindEvents() {
  ...
  bindEntryLauncher('entryTreasuryPublicButton', enterTreasuryFromGate);
  $('treasuryMenuButton')?.addEventListener('click', (event) => { event.preventDefault(); event.stopPropagation(); enterTreasuryFromGate(); });
...
async function init() {
  ...
  setInterval(() => { if (!document.hidden) void loadTreasuryPreview(false); }, 12_000);
...
window.addEventListener('pageshow', (event) => {
  ...
  requestAnimationFrame(() => {
    ...
    window.setTimeout(() => {
      ...
      try { loadTreasuryPreview(true); } catch (_) {}
    }, 180);
  });
});
...
// v15.99.25 — INJ NODE Treasury: public aggregate view backed by a private server-side wallet list.
