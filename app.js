...
function renderEntryGate() {
  ...
  requestAnimationFrame(() => {
    ...
    if (selectedRow && state.wallets.length > 1) selectedRow.scrollIntoView({ behavior: 'auto', block: 'nearest', inline: 'nearest' });
  });
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.81').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
