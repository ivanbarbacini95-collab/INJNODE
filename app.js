...
function closeFocusDisplay({ revealDashboard = true, animate = false } = {}) {
  ...
  const finish = () => {
    ...
    if (!revealDashboard) {
      // Do not detach the fullscreen layer on iOS. Keeping it mounted but parked
      // removes the compositor teardown that caused the black frame.
  ...
  const onEnd = (event) => {
    if (event.target === dialog && event.propertyName === 'opacity') complete();
  };
...
function returnDashboardToEntryGate() {
  ...
  state.currentView = 'home';
  renderEntryGate();
  gate.classList.remove('leaving', 'immersive-underlay', 'mode-return');
  gate.classList.add('visible', 'dashboard-underlay');
  gate.setAttribute('aria-hidden', 'false');

  document.body.classList.remove('entry-live-launch', 'mode-returning');
  document.body.classList.add('dashboard-home-return');

  // Two frames guarantee that Home is already painted below Dashboard before
  // Dashboard starts fading. There is never an uncovered root canvas.
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      document.body.classList.add('dashboard-home-fade');
    });
  });
  ...
  window.setTimeout(() => {
    ...
    document.body.classList.remove('dashboard-home-return', 'dashboard-home-fade');
  }, 560);
}
...
function closePulseView({ revealDashboard = true, animate = false } = {}) {
  ...
  const onEnd = (event) => {
    if (event.target === overlay && event.propertyName === 'opacity') complete();
  };
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.70').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function bindEvents() {
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'Escape' && isFocusDisplayActive()) goHome();
    if (event.key === 'Escape' && isPulseViewActive()) goHome();
  });
