...
function renderPulseMiniChart(id, points) {
  ...
  svg.innerHTML = `<path class="pulse-mini-zero" d="M 4 24 H 176"></path><path class="pulse-mini-line" d="${graph.path}"></path><ellipse class="pulse-mini-dot" cx="${graph.x.toFixed(2)}" cy="${graph.y.toFixed(2)}" rx="${rx.toFixed(3)}" ry="${ry.toFixed(3)}"></ellipse>`;
}
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.99.54').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
