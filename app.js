...
function fillEntryOverviewClone(clone, address) {
  ...
  if (q('entryWalletWorth')) q('entryWalletWorth').textContent = total > 0 && state.price > 0 ? money(worth, 2) : '—';
  ...
  if (q('entryPulsePrice')) q('entryPulsePrice').textContent = state.price > 0 ? money(state.price, state.price < 10 ? 4 : 3) : '—';
  const pulseChange = q('entryPulseChange');
  const pulseChangeValue = Number(state.change);
  if (pulseChange) {
    pulseChange.textContent = state.price > 0 ? `${pulseChangeValue > 0 ? '+' : ''}${pulseChangeValue.toFixed(2)}%` : '—';
    pulseChange.className = pulseChangeValue > 0 ? 'positive' : pulseChangeValue < 0 ? 'negative' : 'neutral';
  }
  const cloneApr = number(row?.personalApr) || (wallet.address === state.address ? number(state.personalApr) : 0) || number(state.networkApr);
  if (q('entryPulseApr')) q('entryPulseApr').textContent = cloneApr > 0 ? `${cloneApr.toFixed(2)}%` : '—';
  const cloneNetwork = q('entryPulseNetwork');
  if (cloneNetwork) {
    const cloneSnapshot = overallSystemSnapshot();
    cloneNetwork.textContent = cloneSnapshot.overall === 'online' ? 'NETWORK LIVE' : cloneSnapshot.overall === 'warn' ? 'NETWORK CHECK' : 'NETWORK OFFLINE';
    cloneNetwork.closest('.entry-market-pulse-network')?.classList.toggle('warn', cloneSnapshot.overall === 'warn');
    cloneNetwork.closest('.entry-market-pulse-network')?.classList.toggle('offline', cloneSnapshot.overall === 'offline');
  }
...
function renderEntryMarketPulse(wallet, row, snapshot) {
  const pulseApr = number(row?.personalApr) || (wallet?.address === state.address ? number(state.personalApr) : 0) || number(state.networkApr);
  const changeValue = Number(state.change);
  setEntryAnimatedValue('entryPulsePrice', state.price > 0 ? money(state.price, state.price < 10 ? 4 : 3) : '—', state.price, { directional: true });
  setEntryAnimatedValue('entryPulseApr', pulseApr > 0 ? `${pulseApr.toFixed(2)}%` : '—', pulseApr);

  const change = $('entryPulseChange');
  if (change) {
    const nextText = state.price > 0 ? `${changeValue > 0 ? '+' : ''}${changeValue.toFixed(2)}%` : '—';
    const changed = change.dataset.entryRenderedValue !== undefined && change.dataset.entryRenderedValue !== nextText;
    change.textContent = nextText;
    change.className = changeValue > 0 ? 'positive' : changeValue < 0 ? 'negative' : 'neutral';
    if (!state.suppressEffects && changed) {
      change.classList.add('entry-value-update');
      clearTimeout(change._entryValueTimer);
      change._entryValueTimer = window.setTimeout(() => change.classList.remove('entry-value-update'), 420);
    }
    change.dataset.entryRenderedValue = nextText;
  }

  const network = $('entryPulseNetwork');
  const networkHost = network?.closest('.entry-market-pulse-network');
  if (network && snapshot) {
    network.textContent = snapshot.overall === 'online' ? 'NETWORK LIVE' : snapshot.overall === 'warn' ? 'NETWORK CHECK' : 'NETWORK OFFLINE';
    networkHost?.classList.toggle('warn', snapshot.overall === 'warn');
    networkHost?.classList.toggle('offline', snapshot.overall === 'offline');
  }
}
...
function renderEntryOverview() {
  ...
  setEntryAnimatedValue('entryWalletWorth', total > 0 && state.price > 0 ? money(worth, 2) : '—', worth);
  ...
  renderEntryMarketPulse(wallet, row, snapshot);
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.97').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
