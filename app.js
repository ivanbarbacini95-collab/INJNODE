...
function selectedEntryApr(row, wallet) {
  const cachedApr = number(row?.personalApr);
  if (cachedApr > 0) return cachedApr;
  if (wallet?.address === state.address && number(state.personalApr) > 0) return number(state.personalApr);
  return 0;
}

function selectedEntryCompoundDays(staked, apr, gain = 1) {
  const principal = Math.max(0, number(staked));
  const dailyRate = Math.max(0, number(apr)) / 100 / 365;
  const targetGain = Math.max(0, number(gain));
  if (!(principal > 0) || !(dailyRate > 0) || !(targetGain > 0)) return 0;
  const days = Math.log((principal + targetGain) / principal) / Math.log(1 + dailyRate);
  return Number.isFinite(days) && days > 0 ? Math.ceil(days) : 0;
}

function entrySparkPath(points) {
  const rows = Array.isArray(points) ? points.filter((row) => number(row?.price) > 0) : [];
  if (rows.length < 2) return { line: '', area: '', direction: 0 };
  ...
  const coords = prices.map((price, index) => {
    ...
    const y = bottom - ((price - min) / span) * (bottom - top);
    return [x, y];
  });
  const line = coords.map(([x, y], index) => `${index ? 'L' : 'M'}${x.toFixed(2)} ${y.toFixed(2)}`).join(' ');
  const area = `${line} L${width} ${height} L0 ${height} Z`;
  return { line, area, direction: prices.at(-1) - prices[0] };
}
...
function renderEntryAtGlance() {
  ...
  const total = number(row?.total);
  const apr = selectedEntryApr(row, wallet);
  const dailyReward = staked > 0 && apr > 0 ? staked * (apr / 100) / 365 : 0;
  const pnl24h = total > 0 && Number.isFinite(Number(state.changeAmount)) ? total * number(state.changeAmount) : 0;

  if ($('entryInsightApr')) $('entryInsightApr').textContent = apr > 0 ? `${apr.toFixed(2)}%` : '—';
  if ($('entryInsightDailyReward')) {
    $('entryInsightDailyReward').textContent = dailyReward > 0
      ? `${dailyReward.toLocaleString('it-IT', { minimumFractionDigits: 4, maximumFractionDigits: 4 })} INJ`
      : '—';
  }
  const pnl = $('entryInsightPnl');
  if (pnl) {
    pnl.textContent = total > 0 && state.price > 0 ? `${pnl24h > 0 ? '+' : ''}${money(pnl24h, 2)}` : '—';
    pnl.className = pnl24h > 0 ? 'positive' : pnl24h < 0 ? 'negative' : 'neutral';
  }

  const days = selectedEntryCompoundDays(staked, apr, 1);
  const eta = rewardEtaCopy(days);
  if ($('entryInsightMilestone')) $('entryInsightMilestone').textContent = days > 0 ? eta.duration : '—';
  if ($('entryInsightMilestoneDate')) {
    $('entryInsightMilestoneDate').textContent = days > 0 ? `Stima · ${eta.date}` : 'Compound giornaliero';
  }
...
async function loadEntrySparkline(force = false) {
  ...
  if (!force && state.entrySpark.loadedAt && Date.now() - state.entrySpark.loadedAt < 2 * 60_000 && state.entrySpark.points.length > 2) {
    renderEntryAtGlance();
    return;
  }
  ...
  try {
    ...
    const data = await fetchJson(`https://api.binance.com/api/v3/klines?symbol=INJUSDT&interval=30m&startTime=${Math.floor(start)}&endTime=${Math.floor(end)}&limit=60`, 9000);
    const rows = Array.isArray(data) ? data.filter((row) => number(row?.[4]) > 0) : [];
    if (rows.length) {
      state.entrySpark.points = rows.map((row) => ({ t: number(row[6]) || number(row[0]), price: number(row[4]) }));
      if (state.price > 0) state.entrySpark.points.push({ t: Date.now(), price: state.price });
      state.entrySpark.loadedAt = Date.now();
    }
  } catch (error) {
    console.warn('Entry sparkline non disponibile', error);
  } finally {
...
// v15.71 — Dashboard Home is intentionally harder to trigger than every other
// header action. A valid navigation must begin on the visible Home tile while
// the Control Center is actually open. This prevents WebKit hit-test/synthetic
// click glitches around the chart/search controls from ever reaching Home.
...
function consumeDashboardHomeIntent(event) {
  ...
  if (!$('headerMenu')?.classList.contains('open')) return false;
...
const state = {
  ...
  entrySpark: { points: [], loadedAt: 0, loading: false },
...
function updateMarket(next) {
  ...
  if ($('entryGate')?.classList.contains('visible')) {
    const lastSpark = state.entrySpark.points.at(-1);
    if (state.price > 0 && (!lastSpark || Date.now() - number(lastSpark.t) > 30_000)) {
      state.entrySpark.points.push({ t: Date.now(), price: state.price });
      if (state.entrySpark.points.length > 80) state.entrySpark.points.splice(0, state.entrySpark.points.length - 80);
    }
    renderEntryOverview();
  }
}
...
function goHome(source = 'internal') {
  ...
  if (state.currentView === 'dashboard' && source !== 'dashboard-menu') return;
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.71').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
