// v15.99.67 — Mobile Home always covers 100dvh, removing the iOS bottom blue/void strip.
// v15.98.67 — Home Market Matrix: live 1M/5M/10M/1H/1D/1W direction consensus.
// v15.99.65 — Command Center mobile notice is rendered as a centered overlay above Home.
...
function enterCommandCenterFromGate() {
  ...
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  window.location.assign(`./command-center.html?wallet=${encodeURIComponent(wallet.address)}&v=15.99.67`);
}
...
function commandCenterUnavailableNotice() {
  ...
  clearTimeout(commandCenterUnavailableNotice.cleanupTimer);
  host.classList.remove('show');
  host.classList.add('command-center-notice');
  host.textContent = 'Command Center può essere aperto solo da PC.';

  requestAnimationFrame(() => host.classList.add('show'));
  toast.timer = setTimeout(() => {
    host.classList.remove('show');
    commandCenterUnavailableNotice.cleanupTimer = setTimeout(() => {
      host.classList.remove('command-center-notice');
    }, 240);
  }, 2200);
}
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.99.67').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
