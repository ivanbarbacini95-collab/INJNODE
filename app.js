...
const state = {
  ...
  focusLiveChart: { samples: [], lastSampleAt: 0, lastTradeAt: 0, renderFrame: 0, lastRenderAt: 0, burstUntil: 0, visualPrice: 0, targetPrice: 0, animateUntil: 0, lastDirection: 0 },
...
const FOCUS_LIVE_WINDOW_MS = 60_000;
const FOCUS_LIVE_MAX_SAMPLES = 420;
...
function updateFocusLiveTick(rawPrice, rawTime = Date.now()) {
  ...
  const cutoff = now - FOCUS_LIVE_WINDOW_MS - 2_500;
...
function buildFocusLiveChartPath(samples, visualPrice = 0) {
  if (!samples.length) return null;
  const latestTradeTime = samples.at(-1).t || Date.now();
  const windowStart = latestTradeTime - FOCUS_LIVE_WINDOW_MS;
  const visible = samples.filter((point) => point.t >= windowStart && point.price > 0).map((point) => ({ ...point }));
  if (!visible.length) return null;
  if (visible.length === 1) visible.unshift({ t: windowStart, price: visible[0].price });
  if (visualPrice > 0) visible[visible.length - 1].price = visualPrice;

  const prices = visible.map((point) => point.price);
  let min = Math.min(...prices);
  let max = Math.max(...prices);
  const latest = visible.at(-1).price;
  const minimumSpan = Math.max(latest * 0.000042, 0.000045);
  let span = max - min;
  if (span < minimumSpan) {
  ...
  const mid = (min + max) / 2;
  const compression = impact * 0.18;
  const renderVisible = focusLiveRenderSamples(visible);
  const coords = renderVisible.map((point, index) => {
    const isLast = index === renderVisible.length - 1;
    const age = Math.max(0, Math.min(1, (latestTradeTime - point.t) / FOCUS_LIVE_WINDOW_MS));
    const historicalFactor = isLast ? 1 : 1 - compression * Math.pow(age, 0.72);
    ...
    // The live endpoint is always pinned to the far-right edge. Older samples
    // occupy the ribbon behind it, making the chart feel continuous rather than
    // leaving the live dot stranded before the edge between sparse trades.
  ...
  return { line, area, last, impact, direction };
}
...
function renderFocusLiveChart(force = false, timestamp = performance.now()) {
  ...
  const path = buildFocusLiveChartPath(chart.samples, chart.visualPrice);
  if (!path) {
    if (state.price > 0) updateFocusLiveTick(state.price, Date.now());
    return false;
  }
...
function scheduleFocusLiveChartRender(force = false) {
  ...
  const frame = (timestamp) => {
    ...
    const minFrameGap = MOBILE_EFFICIENCY ? 30 : 15;
    ...
    const keepAnimating = isFocusDisplayActive() && (
      stillMoving ||
      timestamp < (chart.animateUntil || 0) ||
      (Date.now() - chart.lastTradeAt < 110)
    );
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.37').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
