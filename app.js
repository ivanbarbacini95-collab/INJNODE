...
function renderEntryGate() {
  ...
  if (!shell || !host || !hint || !liveButton || !pulseButton || !liveChartsButton) return;
...
const state = {
  ...
  liveCharts: { samples: [], lastSampleAt: 0, lastTradeAt: 0, renderFrame: 0, lastRenderAt: 0, visualPrice: 0, targetPrice: 0, visualFrameAt: 0, historyLoadedAt: 0, persistAt: 0, scales: { m5: { min: 0, max: 0 }, m10: { min: 0, max: 0 }, h1: { min: 0, max: 0 } } },
...
function updateMarket(next) {
  ...
  if (nextPrice > 0) { state.price = nextPrice; if (Date.now() - number(state.liveCharts?.lastTradeAt) > 1400) updateLiveChartsTick(nextPrice, Date.now()); }
...
function connectMarketSocket() {
  ...
  try {
    ...
    // aggTrade feeds the shared continuous 5m / 10m / 1h Live Charts stream.
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (stream.endsWith('@aggTrade')) {
          updateLiveChartsTick(tick.p, tick.T || tick.E || Date.now());
          return;
        }
        updateLiveChartsTick(tick.c, tick.E || Date.now());
...
const LIVE_CHART_MAX_SAMPLES = 8200;
...
function persistLiveChartsHistory(force = false) {
  ...
  const now = Date.now();
  if (!force && now - number(chart.persistAt) < 20_000) return;
  chart.persistAt = now;
  try {
    const cutoff = now - LIVE_CHART_MAX_WINDOW_MS - 90_000;
    const rows = chart.samples.filter((row) => row.t >= cutoff && row.price > 0);
    const compact = focusLiveRenderSamples(rows, 1600).map((row) => [Math.round(row.t), Number(row.price)]);
...
function updateLiveChartsTick(rawPrice, rawTime = Date.now()) {
  ...
  const minGap = MOBILE_EFFICIENCY ? 500 : 320;
  ...
  persistLiveChartsHistory(false);
...
function buildLiveChartsPath(samples, visualPrice, rangeKey, renderNow, frameGap) {
  ...
  const scale = state.liveCharts.scales[rangeKey];
  ...
  const maxPoints = MOBILE_EFFICIENCY ? 145 : 185;
...
function renderLiveCharts(force = false, timestamp = performance.now()) {
  ...
  const frameGap = Math.max(8, Math.min(60, timestamp - (chart.visualFrameAt || timestamp - 16)));
  ...
  if (Math.abs(delta) <= epsilon) chart.visualPrice = target;
    ...
    const alpha = 1 - Math.exp(-frameGap / (MOBILE_EFFICIENCY ? 68 : 54));
  ...
  for (const [rangeKey, config] of Object.entries(LIVE_CHART_RANGES)) {
    const path = buildLiveChartsPath(chart.samples, chart.visualPrice, rangeKey, renderNow, frameGap);
    if (!path) continue;
    const line = $(`${config.prefix}Line`);
    const area = $(`${config.prefix}Area`);
    const dot = $(`${config.prefix}Dot`);
    const card = $(config.prefix);
    if (!line || !area || !dot || !card) continue;
    ...
    setLiveChartsText(`${config.prefix}Live`, liveChartsPriceText(path.latest));
    const marker = $(`${config.prefix}Marker`);
    if (marker) marker.style.top = `${Math.max(8, Math.min(92, (path.last.y / LIVE_CHART_VIEWBOX_HEIGHT) * 100))}%`;
  }
  return true;
}
...
function scheduleLiveChartsRender(force = false) {
  ...
  const frame = (timestamp) => {
    ...
    const minFrameGap = MOBILE_EFFICIENCY ? 16 : 12;
    if (forceThisFrame || timestamp - chart.lastRenderAt >= minFrameGap) {
      chart.lastRenderAt = timestamp;
      renderLiveCharts(forceThisFrame, timestamp);
    }
    if (isLiveChartsActive()) chart.renderFrame = requestAnimationFrame(frame);
  };
...
function openLiveChartsView() {
  ...
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  if (state.price > 0 && !state.liveCharts.samples.length) updateLiveChartsTick(state.price, Date.now());
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.39').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function bindEvents() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (!event.target?.closest?.('#entryDashboardButton, #entryLiveButton, #entryPulseButton, #entryLiveChartsButton')) return;
    event.preventDefault();
    event.stopImmediatePropagation();
  }, true);
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    void loadLiveChartsHistory(true);
    connectMarketSocket();
    refreshAll(false);
  });
}
