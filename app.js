// v15.98.42 — Live Charts isolated page: Home only navigates; chart rendering can no longer block the main app.
...
function enterLiveChartsFromGate() {
  if (!modeLaunchAllowed()) return;
  // v15.98.42: Live Charts is deliberately isolated in its own lightweight page.
  ...
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  window.location.assign('./live-charts.html?v=15.98.42');
}
...
function connectMarketSocket() {
  ...
  try {
    // Main-app market stream only. Live Charts runs on its own isolated page.
...
// v15.98.42 — Live Charts now runs on an isolated Canvas page to protect the main application thread.
