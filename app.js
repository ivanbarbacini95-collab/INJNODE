// v15.98.10 — Pulse reward odometer is throttled to first-decimal changes; Dashboard/Live remain per-digit live.
'use strict';
...
function appendRollingDigit(parent, oldDigit, newDigit, direction) {
  if (oldDigit === newDigit) {
    parent.append(document.createTextNode(newDigit));
    return;
  }
...
// Pulse rewards increase continuously. Keep the exact 4-decimal text live, but only
// run the odometer when the first decimal digit changes (e.g. 0.1499 -> 0.1500).
// A reward claim naturally crosses buckets in the opposite direction and still rolls down.
function setPulseRewardRollingValue(element, text, numericValue) {
  if (!element) return { visibleChange: false, numericChange: false, previous: NaN, next: NaN };
  ...
  const bucketFor = (value) => Number.isFinite(value)
    ? Math.floor((Math.max(0, value) + 1e-10) * 10)
    : null;
  const previousBucket = element.dataset.pulseRewardRollBucket === undefined
    ? bucketFor(previous)
    : Number(element.dataset.pulseRewardRollBucket);
  const nextBucket = bucketFor(next);
  const shouldRoll = visibleChange && numericChange && previousBucket !== null && nextBucket !== null && previousBucket !== nextBucket;
  ...
  if (nextBucket !== null) element.dataset.pulseRewardRollBucket = String(nextBucket);
  else delete element.dataset.pulseRewardRollBucket;

  return { visibleChange, numericChange, previous, next, previousText, nextText, shouldRoll };
}
...
function renderFocusDisplay() {
  ...
  if (change) {
    ...
    setRollingNumberValue(change, state.price > 0 ? `${next > 0 ? '+' : ''}${next.toFixed(2)}% · 24H` : '— · 24H', state.price > 0 ? next : NaN);
  ...
  if ($('focusDisplayTotalInj')) {
    setRollingNumberValue($('focusDisplayTotalInj'), state.address ? formatInj(total, 4) : '—', state.address ? total : NaN);
  }
  ...
  if ($('focusDisplayRangeMin')) setRollingNumberValue($('focusDisplayRangeMin'), state.low > 0 ? money(state.low, state.low < 10 ? 3 : 2) : '—', state.low > 0 ? state.low : NaN);
  if ($('focusDisplayRangeMax')) setRollingNumberValue($('focusDisplayRangeMax'), state.high > 0 ? money(state.high, state.high < 10 ? 3 : 2) : '—', state.high > 0 ? state.high : NaN);
  ...
  if ($('focusDisplayApr')) setRollingNumberValue($('focusDisplayApr'), state.personalApr > 0 ? `${state.personalApr.toFixed(3)}%` : '—', state.personalApr > 0 ? state.personalApr : NaN);
  if ($('focusDisplayStaked')) setRollingNumberValue($('focusDisplayStaked'), state.address ? formatInj(state.staked, 4) : '—', state.address ? state.staked : NaN);
  if ($('focusDisplayRewards')) setRollingNumberValue($('focusDisplayRewards'), state.address ? formatInj(state.rewards, 4) : '—', state.address ? state.rewards : NaN);
  renderFocusAmbient();
}
...
function setPulseExtremeUI(id, metricKey, row, fullScale, flash = '') {
  ...
  if (high) {
    ...
    if (metricKey === 'reward') setPulseRewardRollingValue(high, text, numeric);
    else setRollingNumberValue(high, text, numeric);
  }
  if (low) {
    const text = hasLow ? pulseExtremeText(metricKey, row.ghostLow) : '—';
    const numeric = hasLow ? Number(row.ghostLow) : NaN;
    if (metricKey === 'reward') setPulseRewardRollingValue(low, text, numeric);
    else setRollingNumberValue(low, text, numeric);
  }
...
function setPulseMetric(id, value, level, deltaText, currentText, metaText) {
  ...
  if (delta) {
    if (rewardMetric) setPulseRewardRollingValue(delta, deltaText, n);
    else setRollingNumberValue(delta, deltaText, n);
  }
  if (current) {
    const currentNumeric = id === 'pulseMetricPrice' ? state.price
      : id === 'pulseMetricPnl' ? currentTotalInj() * state.price
      : state.rewards;
    if (rewardMetric) setPulseRewardRollingValue(current, currentText, currentNumeric);
    else setRollingNumberValue(current, currentText, currentNumeric);
  }
...
function renderPulseView() {
  ...
  if (rewardHigh && rewardExtreme.row.ghostHigh !== null && rewardExtreme.row.ghostHigh !== undefined && Number.isFinite(Number(rewardExtreme.row.ghostHigh))) setRollingNumberValue(rewardHigh, pulseExtremeText('reward', rewardExtreme.row.ghostHigh), Number(rewardExtreme.row.ghostHigh));
  if (rewardLow && rewardExtreme.row.ghostLow !== null && rewardExtreme.row.ghostLow !== undefined && Number.isFinite(Number(rewardExtreme.row.ghostLow))) setRollingNumberValue(rewardLow, pulseExtremeText('reward', rewardExtreme.row.ghostLow), Number(rewardExtreme.row.ghostLow));
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.10').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
// v15.98.10 — Pulse reward roll only on first-decimal boundaries; other odometers unchanged.
