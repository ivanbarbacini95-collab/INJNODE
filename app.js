...
function renderEntryAtGlance() {
  ...
  if (liveDot) {
    if (spark.last) {
      liveDot.style.left = `${(spark.last[0] / spark.width) * 100}%`;
      liveDot.style.top = `${(spark.last[1] / spark.height) * 100}%`;
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.80').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
