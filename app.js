...
function showEntryGate({ modeReturn = false } = {}) {
  ...
  document.body.classList.remove('amber-dashboard-active');
  renderEntryGate();
...
function enterDashboardFromGate() {
  ...
  document.body.classList.toggle('amber-dashboard-active', document.documentElement.dataset.theme === 'amber');
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.25').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function applyTheme(theme, notify = false) {
  const themes = {
    ...
    amber: { label: 'Amber Night', color: '#100904' }
  };
  const selected = themes[theme] ? theme : 'navy';
  document.documentElement.dataset.theme = selected;
  document.body.classList.toggle('amber-dashboard-active', selected === 'amber' && state.currentView === 'dashboard');
...
// v15.98.26 — Amber Night: warm graphite dashboard theme with restrained amber/copper edge light. Home, Live View and Pulse View keep Aurora styling.
