...
// v15.98.5 — Launch sequence: countdown gate -> boot splash -> wallet entry -> Home.
// The app does NOT initialize underneath the countdown gate.
const INJ_NODE_LAUNCH_AT = Date.parse('2026-09-09T09:00:00+02:00');
let launchCountdownTimer = 0;
let launchGateUnlocked = false;
let appBootStarted = false;

function launchPad(value) {
  return Math.max(0, Number(value) || 0).toString().padStart(2, '0');
}

function setLaunchUnderlyingInert(locked) {
  ['bootSplash', 'entryGate'].forEach((id) => {
    const node = $(id);
    if (node) node.inert = Boolean(locked);
  });
  const shell = document.querySelector('.app-shell');
  if (shell) shell.inert = Boolean(locked);
}

function unlockLaunchGate() {
  if (launchGateUnlocked) return;
  launchGateUnlocked = true;
  ...
  const liveState = $('launchLiveState');
  const button = $('launchOpenButton');
  const buttonText = $('launchOpenButtonText');
  const status = $('launchTopStatus');
  const note = $('launchLockNote');

  if (countdown) countdown.hidden = true;
  if (liveState) liveState.hidden = false;
  if (button) button.disabled = false;
  if (buttonText) buttonText.textContent = 'Apri INJ Node';
  if (status) {
    status.classList.add('live');
    const copy = status.querySelector('span');
    if (copy) copy.textContent = 'LIVE NOW';
  }
  if (note) {
    note.classList.add('live');
    note.lastChild.textContent = ' La nuova Dashboard è disponibile';
  }
}

function renderLaunchCountdown() {
  const remaining = INJ_NODE_LAUNCH_AT - Date.now();
  if (remaining <= 0) {
    unlockLaunchGate();
    return;
  }
  ...
  Object.entries(values).forEach(([id, value]) => {
    const node = $(id);
    if (node) node.textContent = launchPad(value);
  });
}

function startLaunchedApp() {
  if (appBootStarted) return;
  appBootStarted = true;
  document.body.classList.remove('launch-locked');
  setLaunchUnderlyingInert(false);

  // The actual app initialization starts only now, while the INJ Node
  // boot splash is visible. When the splash finishes, the wallet entry
  // gate is revealed; only after a wallet is accepted can Home open.
  scheduleBootSplashExit();
  void init();
}

function enterLaunchedApp() {
  if (!launchGateUnlocked || Date.now() < INJ_NODE_LAUNCH_AT || appBootStarted) return;
...
function initLaunchGate() {
  ...
  setLaunchUnderlyingInert(true);
  $('launchOpenButton')?.addEventListener('click', enterLaunchedApp);
  renderLaunchCountdown();
  if (!launchGateUnlocked) launchCountdownTimer = window.setInterval(renderLaunchCountdown, 1000);
}
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.5').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
// Boot + init are intentionally started by the launch gate.
...
// v15.98.4 — launch countdown gate and release highlights.
