...
function renderTreasury() {
  ...
  const previewTotal = $('entryTreasuryTotal');
  const previewWorth = $('entryTreasuryWorth');
  if (!data) {
    if (previewTotal) previewTotal.textContent = state.treasury.loading ? 'SYNC…' : '— INJ';
    if (previewWorth) previewWorth.textContent = state.treasury.loading ? 'Aggiornamento live…' : 'Treasury pubblica';
    return;
  }
  ...
  if (previewTotal) previewTotal.textContent = treasuryInj(total, 2);
  if (previewWorth) previewWorth.textContent = state.price > 0 ? `${money(worth, 2)} · aggregated` : 'Aggregated data · addresses hidden';
...
async function loadTreasury(force = false) {
  ...
  try {
    // Treasury is read directly from Injective's public LCD endpoints.
    // No /api route or Vercel environment variable is required.
    const [walletResults, apr] = await Promise.all([
      Promise.allSettled(TREASURY_ADDRESSES.map((address) => loadWalletSummary(address))),
      ensureNetworkApr(false).catch(() => number(state.networkApr))
    ]);

    const rows = walletResults
    ...
    const detail = 'Rete Injective temporaneamente non disponibile';
    if ($('treasuryUpdated')) $('treasuryUpdated').textContent = detail;
    if (!state.treasury.data && $('entryTreasuryWorth')) $('entryTreasuryWorth').textContent = detail;
  } finally {
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.99.27').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function bindEvents() {
  ...
  bindEntryLauncher('entryTreasuryPreview', enterTreasuryFromGate);
  ...
  document.addEventListener('click', (event) => {
    ...
    if (!event.target?.closest?.('#entryDashboardButton, #entryLiveButton, #entryPulseButton, #entryLiveChartsButton, #entryOrderBookButton, #entryPerformanceButton, #entryTreasuryButton, #entryTreasuryPreview, #entryCommandCenterButton')) return;
    event.preventDefault();
    event.stopImmediatePropagation();
  }, true);
...
async function init() {
  ...
  setInterval(() => { if (!document.hidden) void loadTreasury(false); }, 15_000);
