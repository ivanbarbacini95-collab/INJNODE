...
// v15.99.64 — Command Center restored as a dedicated PC-only section; mobile launcher shows an availability notice.
...
function enterCommandCenterFromGate() {
  ...
  if (!commandCenterDesktopAllowed()) {
    toast('Command Center può essere aperto solo da PC.');
    return;
  }

  setHeaderMenuOpen(false);
  setSearchOpen(false);
  window.location.assign(`./command-center.html?wallet=${encodeURIComponent(wallet.address)}&v=15.99.64`);
}
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.99.59').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
async function init() {
  ...
  try {
    ...
    if (url.searchParams.get('command-center') === 'pc-only') {
      window.setTimeout(() => toast('Command Center può essere aperto solo da PC.'), 80);
