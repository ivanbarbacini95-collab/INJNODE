...
function enterLiveChartsFromGate() {
  ...
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  window.location.assign('./live-charts.html?v=15.98.63');
}
...
const state = {
  ...
  marketMatrix: {
    opens: { m1: 0, m5: 0, m10: 0, h1: 0, w1: 0 },
    bucketStarts: { m1: 0, m5: 0, m10: 0, h1: 0, w1: 0 },
    loadedAt: 0,
    loading: false
  },
...
const MARKET_MATRIX_BUCKETS = { m1: 60_000, m5: 300_000, m10: 600_000, h1: 3_600_000 };
const MARKET_MATRIX_IDS = { m1: 'M1', m5: 'M5', m10: 'M10', h1: 'H1', d1: 'D1', w1: 'W1' };
...
function renderEntryMarketMatrix() {
  ...
  const values = {
    m1: marketMatrixPercent(matrix.opens?.m1, price),
    m5: marketMatrixPercent(matrix.opens?.m5, price),
    m10: marketMatrixPercent(matrix.opens?.m10, price),
  ...
  if (live) {
    live.classList.toggle('online', valid === 6 && price > 0);
    const label = live.querySelector('b');
    if (label) label.textContent = valid === 6 && price > 0 ? 'LIVE' : 'SYNC';
  }
  ...
  if (consensus) {
    ...
    if (!valid) {
      ...
      consensus.textContent = `${bullish}/6 BULLISH`;
      consensus.classList.add('matrix-up');
    } else if (bearish > bullish) {
      consensus.textContent = `${bearish}/6 BEARISH`;
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.98.63').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
