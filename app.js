...
function entrySparkPath(points) {
  ...
  // v15.82 — adaptive Y autoscale with extra breathing room.
  ...
  const padding = rawSpan > 0
    ? Math.max(rawSpan * 0.18, referencePrice * 0.0025)
    : Math.max(referencePrice * 0.004, 0.003);
...
function bindEntrySparkInteraction() {
  ...
  stage.addEventListener('pointerup', (event) => {
    ...
    // Keep the last inspected value visible after the finger is lifted.
  });
...
// v15.82 — Home PRICE taller chart, safe touch tooltip and adaptive autoscaling.
