...
function entrySparkPath(points) {
  ...
  // Autoscaling follows today's true range with breathing room. The x-axis is
  // clock-based, so the line literally grows from 00:00 toward midnight.
  const prices = rows.map((row) => number(row.price));
  const rawMin = Math.min(...prices);
  const rawMax = Math.max(...prices);
  const rawSpan = rawMax - rawMin;
  const padding = rawSpan > 0 ? rawSpan * 0.12 : Math.max(rawMax * 0.0015, 0.0025);
