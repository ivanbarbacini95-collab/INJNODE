...
function renderEntryGate() {
  ...
  const treasuryButton = $('entryTreasuryButton');
...
function paintEntryPnlTimelineScale() {
  ...
  const values = candles.map((node) => number(node.dataset.pnl));
  const maxAbs = Math.max(0.01, ...values.map((value) => Math.abs(value)));
  const stageHeight = Math.max(130, scroll.clientHeight || 190);
  const halfRange = Math.max(42, stageHeight * 0.5 - 34);
  const maxVisual = halfRange * 0.90;
  let visiblePnl = 0;
  let visibleBaseWorth = 0;

  // Only repaint the visible window. Historical candles are recalculated as
  // they enter the viewport, keeping the Home lightweight even after months.
  candles.forEach((node) => {
    const value = number(node.dataset.pnl);
    const ratio = Math.min(1, Math.abs(value) / maxAbs);
    // Linear scaling keeps proportions truthful: small days stay small and
    // large outliers no longer force exaggerated visual compensation.
    ...
    visiblePnl += value;
    visibleBaseWorth += number(node.dataset.baseWorth);
  });
  // The header deliberately shows TODAY only. Scrolling the historical window
  // must never change the day P/L displayed at the top.
  updateEntryPnlTimelineTodayHero();
}
...
function renderEntryPnlTimeline(force = false) {
  ...
  if (force || renderKey !== timeline.renderKey) {
    ...
    const targetVisible = scroll.clientWidth < 620 ? 7.2 : scroll.clientWidth < 980 ? 9.5 : 12.5;
    const slot = Math.max(54, Math.min(124, Math.round(scroll.clientWidth / Math.max(1, targetVisible)) || 72));
    track.style.setProperty('--pnl-slot', `${slot}px`);
    track.style.width = count <= targetVisible ? '100%' : `${Math.max(scroll.clientWidth, count * slot)}px`;
    ...
    rows.forEach((row, index) => {
      ...
      day.addEventListener('click', () => inspectEntryPnlDay(day));
...
function bindEntrySparkInteraction() {
  ...
  stage.addEventListener('keydown', (event) => {
    ...
    if (!samples.length || !['ArrowLeft', 'ArrowRight', 'Home', 'End', 'Escape'].includes(event.key)) return;
    event.preventDefault();
    if (event.key === 'Escape') return hideEntrySparkInspector();
    let index = state.entrySpark.inspectIndex;
    if (event.key === 'Home') index = 0;
    else if (event.key === 'End') index = samples.length - 1;
    else if (event.key === 'ArrowLeft') index = index < 0 ? samples.length - 1 : index - 1;
...
function isTreasuryActive() {
  const view = $('treasuryView');
  return Boolean(view && !view.hidden && view.classList.contains('open'));
}

function treasuryMilestone(total) {
  ...
  return { next, previous, progress: Math.max(0, Math.min(100, ((value - previous) / span) * 100)), remaining: Math.max(0, next - value) };
}

function treasuryInj(value, digits = 4) {
  const v = number(value);
  return `${v.toLocaleString('it-IT', { minimumFractionDigits: digits, maximumFractionDigits: digits })} INJ`;
}

function hydrateTreasuryCache() {
  if (state.treasury.cacheHydrated) return;
  state.treasury.cacheHydrated = true;
  try {
    const cached = JSON.parse(localStorage.getItem(TREASURY_SNAPSHOT_KEY) || 'null');
    if (cached && number(cached.total) > 0) {
      state.treasury.data = cached;
      state.treasury.updatedAt = number(cached.updatedAt);
    }
  } catch (_) {}
}

function persistTreasurySnapshot(data) {
  try { localStorage.setItem(TREASURY_SNAPSHOT_KEY, JSON.stringify(data)); } catch (_) {}
}

function hydrateTreasuryHistory() {
  if (state.treasury.historyLoaded) return;
  state.treasury.historyLoaded = true;
  try {
    const raw = JSON.parse(localStorage.getItem(TREASURY_HISTORY_KEY) || '[]');
    const rows = Array.isArray(raw) ? raw : [];
    const cutoff = Date.now() - 366 * 24 * 60 * 60 * 1000;
    const cleaned = rows
      .map((row) => ({ t: number(row?.t), value: number(row?.value) }))
      .filter((row) => row.t > cutoff && row.value > 0)
      .sort((a,b) => a.t - b.t);
    const deduped = [];
    ...
    state.treasury.samples = deduped.slice(-TREASURY_HISTORY_MAX);
  } catch (_) {
    state.treasury.samples = [];
  }
}

function persistTreasuryHistory() {
  try { localStorage.setItem(TREASURY_HISTORY_KEY, JSON.stringify((state.treasury.samples || []).slice(-TREASURY_HISTORY_MAX))); } catch (_) {}
}

function recordTreasuryHistory(total, at = Date.now()) {
  const value = number(total);
  if (!(value > 0)) return;
  hydrateTreasuryHistory();
  const samples = state.treasury.samples;
  const last = samples.at(-1);
  if (!last) {
    samples.push({ t: at, value });
    persistTreasuryHistory();
    return;
  }

  const gap = Math.max(0, at - number(last.t));
  // When the site was closed, bridge the last real snapshot to the new real
  // snapshot so the timeline remains continuous. Endpoints remain measured;
  // intermediate points are reconstructed between them.
  if (gap > 2 * 60 * 1000) {
    const baseStep = 5 * 60 * 1000;
    ...
    for (let t = number(last.t) + step; t < at; t += step) {
      ...
      samples.push({ t, value: number(last.value) + (value - number(last.value)) * ratio });
    }
  }

  if (gap >= 15_000 || Math.abs(number(last.value) - value) > 1e-10) samples.push({ t: at, value });
  if (samples.length > TREASURY_HISTORY_MAX) samples.splice(0, samples.length - TREASURY_HISTORY_MAX);
  persistTreasuryHistory();
}

function treasuryHistoryLabel(timestamp) {
...
function renderTreasuryMenuSummary() {
  hydrateTreasuryCache();
  const data = state.treasury.data;
  const total = data ? number(data.total) : 0;
  setValue(
    'treasuryMenuTotal',
    total > 0 ? `${total.toLocaleString('it-IT', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} INJ` : 'Apri Treasury',
    total > 0 ? total : NaN,
    { roll: true }
  );
}

function treasuryHistoryFullLabel(timestamp) {
  const t = number(timestamp);
  if (!t) return '—';
  const d = new Date(t);
...
function hideTreasurySparkInspector() {
  const stage = $('treasurySparkStage');
  const cursor = $('treasurySparkCursor');
  const tooltip = $('treasurySparkTooltip');
  if (cursor) cursor.hidden = true;
  if (tooltip) tooltip.hidden = true;
  stage?.classList.remove('inspecting', 'dragging');
  if (state?.treasury) state.treasury.inspectIndex = -1;
}

function showTreasurySparkSample(index) {
  const plot = state.treasury.plot;
  const samples = plot?.samples || [];
  const stage = $('treasurySparkStage');
  const cursor = $('treasurySparkCursor');
  const tooltip = $('treasurySparkTooltip');
  ...
  tooltip.style.left = `${xPct}%`;
  tooltip.style.top = `${yPct}%`;
  tooltip.classList.toggle('flip', xPct > 70);
  tooltip.classList.toggle('below', yPct < 32);
  tooltip.querySelector('strong').textContent = `${number(sample.value).toLocaleString('it-IT', { minimumFractionDigits: 4, maximumFractionDigits: 4 })} INJ`;
  tooltip.querySelector('small').textContent = treasuryHistoryFullLabel(sample.t);
  tooltip.hidden = false;
  stage.classList.add('inspecting');
  state.treasury.inspectIndex = safeIndex;
}

function inspectTreasurySparkAt(event) {
  const stage = $('treasurySparkStage');
  const plot = state.treasury.plot;
  const samples = plot?.samples || [];
  if (!stage || !samples.length) return;
  ...
  const clientX = Number(event?.clientX);
  if (!Number.isFinite(clientX)) return;
  const targetX = Math.max(0, Math.min(plot.width, ((clientX - rect.left) / rect.width) * plot.width));
  let low = 0;
  let high = samples.length - 1;
  while (low < high) {
    const mid = Math.floor((low + high) / 2);
  ...
  let index = low;
  if (index > 0 && Math.abs(samples[index - 1].x - targetX) <= Math.abs(samples[index].x - targetX)) index -= 1;
  showTreasurySparkSample(index);
}

function bindTreasurySparkInteraction() {
  const stage = $('treasurySparkStage');
  if (!stage || stage.dataset.treasurySparkBound === '1') return;
  stage.dataset.treasurySparkBound = '1';
  ...
  const releaseCapture = (pointerId) => {
    try { if (stage.hasPointerCapture?.(pointerId)) stage.releasePointerCapture?.(pointerId); } catch (_) {}
  };

  stage.addEventListener('pointerdown', (event) => {
    if (!state.treasury.plot?.samples?.length) return;
    inspectTreasurySparkAt(event);
    if (event.pointerType === 'mouse') {
      event.preventDefault();
      stage.classList.add('dragging');
      try { stage.setPointerCapture?.(event.pointerId); } catch (_) {}
  ...
  stage.addEventListener('pointermove', (event) => {
    if (event.pointerType === 'mouse') {
      if (stage.classList.contains('inspecting') || stage.hasPointerCapture?.(event.pointerId)) inspectTreasurySparkAt(event);
      return;
    }
    if (event.pointerId !== touch.pointerId) return;
    const dx = event.clientX - touch.startX;
    ...
    if (!touch.horizontal) {
      if (Math.max(ax, ay) < 4) return;
      if (ay > ax * 1.35) { hideTreasurySparkInspector(); resetTouch(); return; }
      if (ax < 4 || ax < ay * 0.72) return;
      touch.horizontal = true;
      stage.classList.add('dragging');
      try { stage.setPointerCapture?.(event.pointerId); } catch (_) {}
  ...
  stage.addEventListener('pointerleave', (event) => {
    if (event.pointerType === 'mouse' && !stage.hasPointerCapture?.(event.pointerId)) hideTreasurySparkInspector();
  });
  stage.addEventListener('pointerup', (event) => {
    inspectTreasurySparkAt(event);
    releaseCapture(event.pointerId);
  ...
  stage.addEventListener('pointercancel', (event) => {
    const keep = touch.horizontal;
    releaseCapture(event.pointerId);
    resetTouch();
    if (!keep) hideTreasurySparkInspector();
  });
  stage.addEventListener('lostpointercapture', () => stage.classList.remove('dragging'));

  document.addEventListener('pointerdown', (event) => {
    if (!stage.classList.contains('inspecting') || stage.contains(event.target)) return;
    hideTreasurySparkInspector();
    resetTouch();
  }, true);

  stage.addEventListener('keydown', (event) => {
    const samples = state.treasury.plot?.samples || [];
    if (!samples.length || !['ArrowLeft','ArrowRight','Home','End','Escape'].includes(event.key)) return;
    event.preventDefault();
    if (event.key === 'Escape') return hideTreasurySparkInspector();
    let index = state.treasury.inspectIndex;
    if (event.key === 'Home') index = 0;
    else if (event.key === 'End') index = samples.length - 1;
    else if (event.key === 'ArrowLeft') index = index < 0 ? samples.length - 1 : index - 1;
    else index = index < 0 ? 0 : index + 1;
...
function renderTreasurySparkline() {
  const svg = $('treasurySparkline');
  if (!svg) return;
  hydrateTreasuryHistory();
  const samples = (state.treasury.samples || []).filter((row) => number(row.t) > 0 && number(row.value) > 0);
  if (samples.length < 2) {
    state.treasury.plot = null;
    svg.innerHTML = '<path d="M0 76 L320 76" opacity=".18"></path>';
    if ($('treasuryHistoryStart')) $('treasuryHistoryStart').textContent = samples.length ? treasuryHistoryLabel(samples[0].t) : 'Prima lettura';
    if ($('treasuryHistoryEnd')) $('treasuryHistoryEnd').textContent = 'Ora';
    hideTreasurySparkInspector();
    return;
  }
  ...
  const plotted = samples.map((row) => ({
    ...
    y: 79 - ((number(row.value) - min) / span) * 61
  }));
  ...
  state.treasury.plot = { width: 320, height: 96, samples: plotted };
  if ($('treasuryHistoryStart')) $('treasuryHistoryStart').textContent = treasuryHistoryLabel(samples[0].t);
  if ($('treasuryHistoryEnd')) $('treasuryHistoryEnd').textContent = treasuryHistoryLabel(samples.at(-1).t);
  if (state.treasury.inspectIndex >= 0) showTreasurySparkSample(Math.min(state.treasury.inspectIndex, plotted.length - 1));
}
...
function renderTreasury() {
  ...
  const data = state.treasury.data;
  if (!data) {
    renderTreasurySparkline();
    return;
  }

  const total = number(data.total);
  const available = number(data.available);
  const staked = number(data.staked);
  const rewards = number(data.rewards);
  const apr = number(data.apr);
  const worth = total * number(state.price);
  const totalSafe = Math.max(total, 0.00000001);
  const stakedPct = Math.max(0, Math.min(100, staked / totalSafe * 100));
  const availablePct = Math.max(0, Math.min(100, available / totalSafe * 100));
  const rewardsPct = Math.max(0, Math.min(100, rewards / totalSafe * 100));
  ...
  const milestone = treasuryMilestone(total);

  // v15.99.31 — Treasury values use the exact same live-number engine as the
  // INJ price in Dashboard: per-digit roll + directional up/down pulse.
  const totalFixed = Math.max(0, total).toFixed(4);
  const [totalWholeRaw, totalDecimalsRaw = '0000'] = totalFixed.split('.');
  const totalWhole = Number(totalWholeRaw).toLocaleString('it-IT');
  setValue('treasuryTotalMajor', totalWhole, total, { roll: true });
  setValue('treasuryTotalDecimals', `,${totalDecimalsRaw}`, total, { roll: true });
  setValue('treasuryTotalWorth', state.price > 0 ? money(worth, 2) : 'Prezzo in sincronizzazione', state.price > 0 ? worth : NaN, { roll: true });

  setValue('treasuryRewards', treasuryInj(rewards, 4), rewards, { roll: true });
  setValue('treasuryRewardsWorth', state.price > 0 ? money(rewards * state.price, 2) : '—', state.price > 0 ? rewards * state.price : NaN, { roll: true });
  setValue('treasuryAvailable', treasuryInj(available, 4), available, { roll: true });
  setValue('treasuryAvailableWorth', state.price > 0 ? money(available * state.price, 2) : '—', state.price > 0 ? available * state.price : NaN, { roll: true });
  setValue('treasuryStaked', treasuryInj(staked, 4), staked, { roll: true });
  setValue('treasuryStakedWorth', state.price > 0 ? money(staked * state.price, 2) : '—', state.price > 0 ? staked * state.price : NaN, { roll: true });
  setValue('treasuryStakeRatio', `${stakedPct.toFixed(1)}%`, stakedPct, { roll: true });

  setValue('treasuryStakedPct', `${stakedPct.toFixed(1)}%`, stakedPct, { roll: true });
  setValue('treasuryAvailablePct', `${availablePct.toFixed(1)}%`, availablePct, { roll: true });
  setValue('treasuryRewardsPct', `${rewardsPct.toFixed(2)}%`, rewardsPct, { roll: true });
  if ($('treasuryStakedBar')) $('treasuryStakedBar').style.width = `${stakedPct}%`;
  if ($('treasuryAvailableBar')) $('treasuryAvailableBar').style.width = `${availablePct}%`;
  if ($('treasuryRewardsBar')) $('treasuryRewardsBar').style.width = `${Math.max(rewardsPct, rewards > 0 ? .35 : 0)}%`;
  setValue('treasuryApr', apr > 0 ? `APR RETE ${apr.toFixed(3)}%` : 'APR —', apr > 0 ? apr : NaN, { roll: true });

  setValue('treasuryRewardHour', treasuryInj(rewardValues.hour, 5), rewardValues.hour, { roll: true });
  setValue('treasuryRewardDay', treasuryInj(rewardValues.day, 4), rewardValues.day, { roll: true });
  setValue('treasuryRewardMonth', treasuryInj(rewardValues.month, 3), rewardValues.month, { roll: true });
  setValue('treasuryRewardYear', treasuryInj(rewardValues.year, 2), rewardValues.year, { roll: true });
  setValue('treasuryRewardHourWorth', state.price > 0 ? money(rewardValues.hour * state.price, 2) : '—', state.price > 0 ? rewardValues.hour * state.price : NaN, { roll: true });
  setValue('treasuryRewardDayWorth', state.price > 0 ? money(rewardValues.day * state.price, 2) : '—', state.price > 0 ? rewardValues.day * state.price : NaN, { roll: true });
  setValue('treasuryRewardMonthWorth', state.price > 0 ? money(rewardValues.month * state.price, 2) : '—', state.price > 0 ? rewardValues.month * state.price : NaN, { roll: true });
  setValue('treasuryRewardYearWorth', state.price > 0 ? money(rewardValues.year * state.price, 2) : '—', state.price > 0 ? rewardValues.year * state.price : NaN, { roll: true });

  setValue('treasuryMilestoneTitle', `${milestone.next.toLocaleString('it-IT')} INJ`, milestone.next, { roll: true });
  setValue('treasuryMilestonePct', `${milestone.progress.toFixed(1)}%`, milestone.progress, { roll: true });
  if ($('treasuryMilestoneBar')) $('treasuryMilestoneBar').style.width = `${milestone.progress}%`;
  setValue('treasuryMilestoneCurrent', `${total.toLocaleString('it-IT',{maximumFractionDigits:2})} INJ`, total, { roll: true });
  setValue('treasuryMilestoneRemaining', `${milestone.remaining.toLocaleString('it-IT',{maximumFractionDigits:2})} INJ mancanti`, milestone.remaining, { roll: true });

  if ($('treasuryWalletCount')) $('treasuryWalletCount').textContent = data.walletCount > 0 ? (number(data.liveWallets) < number(data.walletCount) ? `${data.liveWallets}/${data.walletCount} wallet live` : `${data.walletCount} wallet`) : '—';
  const updated = number(data.updatedAt) || state.treasury.updatedAt;
  if ($('treasuryUpdated')) $('treasuryUpdated').textContent = updated ? `UPDATE ${new Date(updated).toLocaleTimeString('it-IT',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}` : 'SYNC';

  const samples = state.treasury.samples || [];
  const base = samples.length ? number(samples[0].value) : total;
  const delta = total - base;
  setValue(
    'treasurySessionDelta',
    `${delta >= 0 ? '+' : ''}${delta.toLocaleString('it-IT',{minimumFractionDigits:4,maximumFractionDigits:4})} INJ`,
    delta,
    { roll: true }
  );
  renderTreasurySparkline();
...
async function treasuryFetchJson(url, timeout = 6500) {
...
async function treasuryLcd(path) {
  let lastError;
  for (const base of LCD_ENDPOINTS) {
    try {
      return await treasuryFetchJson(base + path);
    } catch (error) {
      lastError = error;
    }
  }
...
async function loadTreasuryWalletSummary(address) {
  // Reuse a recent Home/Dashboard snapshot when this visitor already tracks
  // the same public address. This avoids duplicating network work.
  const cached = state.walletSummaries[address];
  if (cached && Date.now() - number(cached.updated) < 20_000) return cached;

  const [bank, delegations, rewards] = await Promise.all([
    treasuryLcd(`/cosmos/bank/v1beta1/balances/${address}`),
    treasuryLcd(`/cosmos/staking/v1beta1/delegations/${address}`),
    treasuryLcd(`/cosmos/distribution/v1beta1/delegators/${address}/rewards`)
  ]);
...
async function loadTreasuryRows() {
  const rows = [];
  let cursor = 0;
  // At most two Treasury wallets are queried at once. The main INJ Node data
  // pipeline therefore stays responsive even on iPhone/PWA connections.
  const worker = async () => {
    while (cursor < TREASURY_ADDRESSES.length) {
      const address = TREASURY_ADDRESSES[cursor++];
      try { rows.push(await loadTreasuryWalletSummary(address)); } catch (_) {}
    }
  };
...
async function loadTreasury(force = false) {
  if (state.treasury.loading) return state.treasury.data;
  if (!force && state.treasury.updatedAt && Date.now() - state.treasury.updatedAt < 15_000) return state.treasury.data;

  state.treasury.loading = true;
  const badge = $('treasuryLiveBadge');
  badge?.classList.add('sync');
  ...
  try {
    const [rows, apr] = await Promise.all([
      loadTreasuryRows(),
      ensureNetworkApr(false).catch(() => number(state.networkApr))
    ]);
    ...
    const data = {
      ...
      apr: number(apr),
      walletCount: TREASURY_ADDRESSES.length,
      liveWallets: rows.length,
      updatedAt: Date.now()
    };

    state.treasury.data = data;
    state.treasury.updatedAt = data.updatedAt;
    persistTreasurySnapshot(data);
    recordTreasuryHistory(number(data.total), data.updatedAt);
    renderTreasuryMenuSummary();

    badge?.classList.remove('sync','error');
  } catch (error) {
    badge?.classList.remove('sync');
    badge?.classList.add('error');
    if ($('treasuryUpdated')) $('treasuryUpdated').textContent = 'Rete Injective temporaneamente non disponibile';
  } finally {
    state.treasury.loading = false;
    renderTreasury();
  }

  return state.treasury.data;
}

function enterTreasuryFromGate() {
  if (!modeLaunchAllowed()) return;
  const view = $('treasuryView');
  if (!view) return;
  setHeaderMenuOpen(false);
  setSearchOpen(false);
  parkEntryGateUnderImmersive();
  state.currentView = 'treasury';
  view.hidden = false;
  view.setAttribute('aria-hidden','false');
  renderTreasury();
  requestAnimationFrame(() => view.classList.add('open'));
  window.clearInterval(state.treasury.timer);
  state.treasury.timer = window.setInterval(() => {
    if (!document.hidden && isTreasuryActive()) void loadTreasury(false);
  }, 15_000);
  void loadTreasury(true);
}

function closeTreasuryToHome() {
  const view = $('treasuryView');
  if (!view || view.hidden) return;
  window.clearInterval(state.treasury.timer);
  state.treasury.timer = 0;
  state.currentView = 'home';
  armModeExitLock(650);
  view.classList.remove('open');
  view.setAttribute('aria-hidden','true');
...
// INJ NODE Treasury — public aggregate.
// These public blockchain addresses are intentionally kept out of the UI,
// but they are part of the client bundle and therefore are not secret.
...
const TREASURY_HISTORY_KEY = 'inj_node_treasury_growth_v1';
const TREASURY_SNAPSHOT_KEY = 'inj_node_treasury_snapshot_v1';
const TREASURY_HISTORY_MAX = 2600;
...
const state = {
  ...
  entryPnlTimeline: { address: '', rows: [], renderKey: '', frame: 0, saveAt: 0, lastPaintAt: 0, followLatest: true, inspectDay: 0, rolloverTimer: 0, rolloverAt: 0, lastHydrateAttempt: 0 },
  ...
  treasury: { data: null, loading: false, updatedAt: 0, samples: [], historyLoaded: false, cacheHydrated: false, timer: 0, plot: null, inspectIndex: -1 },
  endpoint: '',
  socket: null,
  walletRequest: 0,
  ...
  pulseMini: { key: '', address: '', price: [], pnl: [], reward: [], lastSampleAt: 0, lastSavedAt: 0 },
...
function ensurePulseMiniKey() {
  ...
  state.pulseMini = saved || { key, address: state.address, price: [], pnl: [], reward: [], lastSampleAt: 0, lastSavedAt: 0 };
  for (const metricKey of ['price', 'pnl', 'reward']) {
...
function savePulseMini(force = false) {
  ...
  try {
    ...
    all[row.key] = {
      ...
      price: row.price.slice(-220), pnl: row.pnl.slice(-220), reward: row.reward.slice(-220),
      lastSampleAt: row.lastSampleAt, lastSavedAt: now
    };
...
function pulseMiniPath(points, width = 180, height = 48) {
  ...
  return { path: d, x: last.x, y: last.y };
}
...
function renderPulseMiniChart(id, points) {
  ...
  if (!graph.path) { svg.innerHTML = ''; return; }
  svg.innerHTML = `<path class="pulse-mini-zero" d="M 4 24 H 176"></path><path class="pulse-mini-line" d="${graph.path}"></path><circle class="pulse-mini-dot" cx="${graph.x.toFixed(2)}" cy="${graph.y.toFixed(2)}" r="2.5"></circle>`;
}

function renderPulseMiniCharts() {
  ensurePulseMiniKey();
  renderPulseMiniChart('pulsePriceSparkline', state.pulseMini.price);
  renderPulseMiniChart('pulsePnlSparkline', state.pulseMini.pnl);
  renderPulseMiniChart('pulseRewardSparkline', state.pulseMini.reward);
}
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.99.28').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function bindEvents() {
  ...
  bindEntryLauncher('entryTreasuryButton', enterTreasuryFromGate);
  ...
  $('closeTreasuryButton')?.addEventListener('click', (event) => {
    event.preventDefault();
    event.stopPropagation();
    closeTreasuryToHome();
  });
  document.addEventListener('click', (event) => {
    ...
    if (!event.target?.closest?.('#entryDashboardButton, #entryLiveButton, #entryPulseButton, #entryLiveChartsButton, #entryOrderBookButton, #entryPerformanceButton, #entryTreasuryButton, #entryCommandCenterButton')) return;
    event.preventDefault();
    event.stopImmediatePropagation();
  }, true);
  ...
  document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && isTreasuryActive()) { closeTreasuryToHome(); return; }
...
// v15.99.28 — Treasury isolation: corrected overlay stacking, removed duplicate Home preview, on-demand refresh only.
