...
function renderDashboardHero() {
  ...
  const netWorth = totalInj * number(state.price);
  const dailyReward = state.staked > 0 && state.personalApr > 0
    ? state.staked * (state.personalApr / 100) / 365
    : 0;
  const dailyRewardUsd = dailyReward * number(state.price);
  const change = number(state.change);
  const netWorthChange24h = totalInj * number(state.changeAmount);

  const walletLabel = $('dashboardHeroWallet');
  if (walletLabel) walletLabel.textContent = activeWallet
    ? `${activeWallet.label} · ${shortAddress(state.address)}`
    : state.address ? shortAddress(state.address) : 'Nessun wallet';

  setValue('dashboardHeroWorth', state.address && state.price > 0 ? money(netWorth, 2) : '—', state.address && state.price > 0 ? netWorth : NaN, { roll: true, flash: false });
  setValue('dashboardHeroStaked', state.address ? formatInj(state.staked, 4) : '—', state.address ? state.staked : NaN, { flash: false });
  setValue('dashboardHeroPrice', state.price > 0 ? money(state.price, state.price < 10 ? 4 : 2) : '—', state.price > 0 ? state.price : NaN, { flash: false });
  setValue('dashboardHeroDailyReward', state.address && dailyReward > 0 ? `${dailyReward.toLocaleString('it-IT', { minimumFractionDigits: 4, maximumFractionDigits: 4 })} INJ` : '—', dailyReward > 0 ? dailyReward : NaN, { flash: false });
  setValue('dashboardHeroDailyRewardUsd', dailyRewardUsd > 0 ? `${money(dailyRewardUsd, dailyRewardUsd < 10 ? 3 : 2)} / giorno` : '—', dailyRewardUsd > 0 ? dailyRewardUsd : NaN, { flash: false });

  const apr = $('dashboardHeroApr');
  if (apr) apr.textContent = state.personalApr > 0 ? `APR ${state.personalApr.toFixed(3)}%` : 'APR —';

  const heroChange = $('dashboardHeroChange');
  if (heroChange) {
    heroChange.textContent = state.address && state.price > 0
      ? `${netWorthChange24h > 0 ? '+' : netWorthChange24h < 0 ? '−' : ''}${money(Math.abs(netWorthChange24h), 2)} · ${change > 0 ? '+' : ''}${change.toFixed(2)}% 24H`
      : '— · 24H';
    heroChange.className = change > 0 ? 'positive' : change < 0 ? 'negative' : 'neutral';
  }

  const priceChange = $('dashboardHeroPriceChange');
  if (priceChange) {
    priceChange.textContent = state.price > 0 ? `${change > 0 ? '+' : ''}${change.toFixed(2)}% · 24H` : '— · 24H';
    priceChange.className = change > 0 ? 'positive' : change < 0 ? 'negative' : 'neutral';
  }
}

function renderPortfolioMarketMotion() {
  renderDashboardHero();
  renderEssentialDashboardDetails();
...
function renderEssentialDashboardDetails() {
  const price = number(state.price);
  const change = number(state.change);

  const livePrice = $('dashboardLivePrice');
  if (livePrice) {
    setBlinkOnlyElement(
      livePrice,
      price > 0 ? `$${price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: price < 10 ? 4 : 2 })}` : '—',
      price > 0 ? price : NaN,
      { flash: false }
    );
  }

  const livePerformance = $('dashboardLivePerformance');
  if (livePerformance) {
    livePerformance.textContent = price > 0 ? `${change > 0 ? '+' : ''}${change.toFixed(2)}% · 24H` : '— · 24H';
    livePerformance.className = change > 0 ? 'positive' : change < 0 ? 'negative' : 'neutral';
  }

  const freshness = $('dashboardFreshness');
  if (freshness) freshness.textContent = summaryAge(state.lastWalletUpdate);

  const rewardText = state.address ? `+${number(state.rewards).toLocaleString('it-IT', { minimumFractionDigits: 4, maximumFractionDigits: 4 })} INJ` : '+0,0000 INJ';
  setValue('stakingRewardGain', rewardText, state.address ? number(state.rewards) : NaN, { flash: false });
  setValue('stakingRewardUsd', state.address && price > 0 ? money(number(state.rewards) * price) : '—', state.address && price > 0 ? number(state.rewards) * price : NaN, { flash: false });

  // v15.98.109 — compact, data-rich essential dashboard.
  setValue('worthAvailableMini', state.address ? formatInj(state.available, 4) : '—', state.address ? state.available : NaN, { flash: false });
  setValue('worthAvailableMiniUsd', state.address && price > 0 ? money(state.available * price) : '—', state.address && price > 0 ? state.available * price : NaN, { flash: false });
  setValue('worthStakedMini', state.address ? formatInj(state.staked, 4) : '—', state.address ? state.staked : NaN, { flash: false });
  setValue('worthStakedMiniUsd', state.address && price > 0 ? money(state.staked * price) : '—', state.address && price > 0 ? state.staked * price : NaN, { flash: false });
  setValue('worthRewardsMini', state.address ? `+${number(state.rewards).toLocaleString('it-IT', { minimumFractionDigits: 4, maximumFractionDigits: 4 })} INJ` : '—', state.address ? state.rewards : NaN, { flash: false });
  setValue('worthRewardsMiniUsd', state.address && price > 0 ? `+${money(state.rewards * price)}` : '—', state.address && price > 0 ? state.rewards * price : NaN, { flash: false });

  const rewardDaily = state.staked > 0 && state.personalApr > 0 ? state.staked * (state.personalApr / 100) / 365 : 0;
  setValue('delegationTotal', state.address ? formatInj(state.staked, 4) : '—', state.address ? state.staked : NaN, { flash: false });
  setValue('delegationTotalUsd', state.address && price > 0 ? money(state.staked * price) : '—', state.address && price > 0 ? state.staked * price : NaN, { flash: false });
  setValue('delegationApr', state.personalApr > 0 ? `${state.personalApr.toFixed(3)}%` : '—', state.personalApr || NaN, { flash: false });
  setValue('delegationRewards', state.address ? rewardText : '—', state.address ? state.rewards : NaN, { flash: false });
  setValue('delegationRewardsUsd', state.address && price > 0 ? `+${money(state.rewards * price)}` : '—', state.address && price > 0 ? state.rewards * price : NaN, { flash: false });
  setValue('delegationDaily', rewardDaily > 0 ? formatInj(rewardDaily, 4) : '—', rewardDaily > 0 ? rewardDaily : NaN, { flash: false });
  setValue('delegationDailyUsd', rewardDaily > 0 && price > 0 ? money(rewardDaily * price, 3) : '—', rewardDaily > 0 && price > 0 ? rewardDaily * price : NaN, { flash: false });
  setValue('delegationCommission', state.staked > 0 ? `${(state.weightedCommission * 100).toFixed(2)}%` : '—', state.staked > 0 ? state.weightedCommission * 100 : NaN, { flash: false });
}
...
function bindEvents() {
  ...
  // v15.98.109 — Asset and Reward Production are fixed dashboard sections, not accordions.
...
async function init() {
  ...
  // Essential dashboard: the three core wallet metrics are always visible.
  try { localStorage.removeItem('inj_node_dashboard_wallet_collapsed_v1'); } catch (_) {}
  setDashboardCollapsed('wallet', false, false);
  try { localStorage.removeItem('inj_node_dashboard_staking_collapsed_v1'); } catch (_) {}
  setDashboardCollapsed('staking', false, false);
  ...
  // Delegations are an essential dashboard block: open by default after the redesign.
  try { localStorage.removeItem('inj_node_dashboard_validators_collapsed_v1'); } catch (_) {}
  setDashboardCollapsed('validators', false, false);
...
// v15.98.109 — Dashboard: patrimonio + distribuzione asset + delegazione fissa + reward production.
