...
function renderEntryGate() {
  ...
  const whaleButton = $('entryWhaleButton');
  if (!shell || !host || !hint || !liveButton || !pulseButton || !whaleButton) return;
...
function registerPwa() {
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => navigator.serviceWorker.register('./sw.js?v=15.97.1').catch(() => {}), { once: true });
  }
  updateInstallUi();
}
...
function bindEvents() {
  ...
  bindEntryLauncher('entryWhaleButton', () => {
    if (!state.address || !state.wallets.length || document.body.classList.contains('boot-loading')) return;
    openWhaleView();
  });
  document.addEventListener('click', (event) => {
    if (Date.now() >= Number(state.modeExitLockUntil || 0)) return;
    if (!event.target?.closest?.('#entryDashboardButton, #entryLiveButton, #entryPulseButton, #entryWhaleButton')) return;
    event.preventDefault();
...
function openWhaleView(){
  const d=$('whaleViewDialog');
  if(!d || !state.address || !state.wallets.length) return;
  const row=state.walletSummaries[state.address]||{};
  const total=number(row.total);
  const worth=total*state.price;
  $('whaleViewValue').textContent=worth?money(worth,2):'—';
  $('whaleViewStaked').textContent=formatInj(number(row.staked),4);
  $('whaleViewRewards').textContent=formatInj(number(row.rewards),4);
  $('whaleViewApr').textContent=state.apr?`${number(state.apr).toFixed(2)}%`:'—';
  $('whaleViewGrowth').textContent=state.apr?`${formatInj(total*number(state.apr)/100,2)} INJ/anno`:'—';
  d.hidden=false; document.body.classList.add('whale-view-open');
}
function closeWhaleView(){
 const d=$('whaleViewDialog'); if(!d)return; d.hidden=true; document.body.classList.remove('whale-view-open');
}
