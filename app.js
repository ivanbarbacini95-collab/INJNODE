...
function renderTreasurySparkline() {
  ...
  if (samples.length < 2) {
    svg.innerHTML = '<path d="M0 68 L320 68" opacity=".18"></path>';
    return;
  }
  ...
  const points = values.map((value, index) => {
    const x = samples.length === 1 ? 160 : (index / (samples.length - 1)) * 320;
    const y = 72 - ((value - min) / span) * 56;
    return [x, y];
  });
  const d = points.map(([x,y],index) => `${index ? 'L' : 'M'}${x.toFixed(2)} ${y.toFixed(2)}`).join(' ');
  const [lastX,lastY] = points[points.length - 1];
  svg.innerHTML = `<path d="${d}"></path><circle cx="${lastX.toFixed(2)}" cy="${lastY.toFixed(2)}" r="3"></circle>`;
}
...
function renderTreasury() {
  ...
  if ($('treasuryTotalInj')) $('treasuryTotalInj').textContent = total.toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 4 });
  if ($('treasuryTotalWorth')) $('treasuryTotalWorth').textContent = state.price > 0 ? money(worth, 2) : 'Prezzo in sincronizzazione';
  if ($('treasuryRewards')) $('treasuryRewards').textContent = treasuryInj(rewards, 4);
  if ($('treasuryRewardsWorth')) $('treasuryRewardsWorth').textContent = state.price > 0 ? money(rewards * state.price, 2) : '—';
  if ($('treasuryAvailable')) $('treasuryAvailable').textContent = treasuryInj(available, 4);
  if ($('treasuryAvailableWorth')) $('treasuryAvailableWorth').textContent = state.price > 0 ? money(available * state.price, 2) : '—';
  if ($('treasuryStaked')) $('treasuryStaked').textContent = treasuryInj(staked, 4);
  if ($('treasuryStakedWorth')) $('treasuryStakedWorth').textContent = state.price > 0 ? money(staked * state.price, 2) : '—';
  if ($('treasuryStakeRatio')) $('treasuryStakeRatio').textContent = `${stakedPct.toFixed(1)}%`;
  if ($('treasuryStakedPct')) $('treasuryStakedPct').textContent = `${stakedPct.toFixed(1)}%`;
  if ($('treasuryAvailablePct')) $('treasuryAvailablePct').textContent = `${availablePct.toFixed(1)}%`;
  if ($('treasuryRewardsPct')) $('treasuryRewardsPct').textContent = `${rewardsPct.toFixed(2)}%`;
  ...
  if ($('treasuryApr')) $('treasuryApr').textContent = apr > 0 ? `APR RETE ${apr.toFixed(3)}%` : 'APR —';
  if ($('treasuryRewardHour')) $('treasuryRewardHour').textContent = treasuryInj(daily / 24, 5);
  if ($('treasuryRewardDay')) $('treasuryRewardDay').textContent = treasuryInj(daily, 4);
  if ($('treasuryRewardMonth')) $('treasuryRewardMonth').textContent = treasuryInj(daily * 30.4375, 3);
  if ($('treasuryRewardYear')) $('treasuryRewardYear').textContent = treasuryInj(daily * 365, 2);
  if ($('treasuryMilestoneTitle')) $('treasuryMilestoneTitle').textContent = `${milestone.next.toLocaleString('it-IT')} INJ`;
  if ($('treasuryMilestonePct')) $('treasuryMilestonePct').textContent = `${milestone.progress.toFixed(1)}%`;
  if ($('treasuryMilestoneBar')) $('treasuryMilestoneBar').style.width = `${milestone.progress}%`;
  if ($('treasuryMilestoneCurrent')) $('treasuryMilestoneCurrent').textContent = `${total.toLocaleString('it-IT',{maximumFractionDigits:2})} INJ`;
  if ($('treasuryMilestoneRemaining')) $('treasuryMilestoneRemaining').textContent = `${milestone.remaining.toLocaleString('it-IT',{maximumFractionDigits:2})} INJ mancanti`;
  if ($('treasuryWalletCount')) $('treasuryWalletCount').textContent = data.walletCount > 0 ? (number(data.liveWallets) < number(data.walletCount) ? `${data.liveWallets}/${data.walletCount} wallet live` : `${data.walletCount} wallet aggregati`) : 'Aggregato';
  ...
  const base = state.treasury.sessionBaseRewards == null ? rewards : number(state.treasury.sessionBaseRewards);
  if ($('treasurySessionDelta')) $('treasurySessionDelta').textContent = `${rewards - base >= 0 ? '+' : ''}${(rewards-base).toFixed(4)} INJ`;
...
async function loadTreasury(force = false) {
  ...
  try {
    ...
    if (state.treasury.sessionBaseRewards == null) state.treasury.sessionBaseRewards = number(data.rewards);

    const samples = state.treasury.samples;
    const last = samples.at(-1);
    if (!last || Math.abs(number(last.value) - number(data.rewards)) > 1e-12 || Date.now() - number(last.t) > 15000) {
      samples.push({ t: Date.now(), value: number(data.rewards) });
...
const state = {
  ...
  treasury: { data: null, loading: false, updatedAt: 0, samples: [], sessionBaseRewards: null, timer: 0 },
  endpoint: '',
  socket: null,
  walletRequest: 0,
