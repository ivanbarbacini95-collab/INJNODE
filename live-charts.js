...
// v15.98.61 — Live Charts: minimal candle-synchronised horizontal time axis for every timeframe.
// v15.98.61 — Live Charts: yellow candle-width Price Tick replaces the live dot/connector; flashes only on new market data.
// v15.98.61 — Global Market Motion parity: isolated Live Charts now interpolates the live candle/header with the same 125 ms response used by Command Center.
// v15.98.61 — Mobile Live Charts Candles.
// v15.98.61 — adds 1 MIN with the same realtime OHLC + yellow Price Tick behavior.
...
function drawRangeTags(ctx,candles,xs,toY,right,bottom){
  ...
  const lx=Math.min(right-12,xs[loI]+4),ly=Math.min(bottom-4,toY(candles[loI].l)+8);ctx.fillText('L',lx,ly);ctx.restore();
}
function pad2(v){return String(v).padStart(2,'0');}
function timeAxisText(ts,key){
  const d=new Date(ts),hh=pad2(d.getHours()),mm=pad2(d.getMinutes()),ss=pad2(d.getSeconds());
  if(key==='m1')return `${hh}:${mm}:${ss}`;
  if(key==='w1'){
    const day=['DOM','LUN','MAR','MER','GIO','VEN','SAB'][d.getDay()];
    return `${day} ${pad2(d.getDate())} · ${hh}`;
  }
  return `${hh}:${mm}`;
}
function drawTimeAxis(ctx,key,candles,xs,left,right,bottom,h){
  if(candles.length<2||xs.length!==candles.length)return;
  const axisTop=bottom+5;
  const axisTextY=h-(mobile?3.5:4);
  const minGap=key==='m1'?(mobile?48:56):(key==='w1'?(mobile?58:68):(mobile?42:50));
  const usable=Math.max(1,right-left);
  const wanted=Math.max(2,Math.min(mobile?4:5,Math.floor(usable/minGap)+1));
  const chosen=[];
  for(let n=0;n<wanted;n++){
    const i=Math.round((candles.length-1)*(n/(wanted-1)));
    if(!chosen.includes(i))chosen.push(i);
  }
  ctx.save();
  ctx.strokeStyle=hexAlpha(state.colors.muted2,.12);
  ctx.lineWidth=.75;
  ctx.beginPath();ctx.moveTo(left,axisTop);ctx.lineTo(right,axisTop);ctx.stroke();
  ctx.font=`650 ${mobile?6.2:7.2}px -apple-system,BlinkMacSystemFont,"SF Pro Display",system-ui,sans-serif`;
  ctx.fillStyle=hexAlpha(state.colors.muted2,.52);
  ctx.textBaseline='alphabetic';
  for(const i of chosen){
    const x=xs[i],label=timeAxisText(candles[i].t,key);
    ctx.strokeStyle=hexAlpha(state.colors.muted2,.18);
    ctx.beginPath();ctx.moveTo(x,axisTop);ctx.lineTo(x,axisTop+2.5);ctx.stroke();
    const tw=ctx.measureText(label).width;
    const tx=Math.max(left+tw/2,Math.min(right-tw/2,x));
    ctx.textAlign='center';ctx.fillText(label,tx,axisTextY);
  }
  ctx.restore();
}
...
function drawOne(now,dt,key,cfg,ts){
  ...
  const axisH=mobile?15:17;
  const left=mobile?8:11,right=w-(mobile?10:13),top=9,bottom=h-axisH-7,plotH=Math.max(1,bottom-top),plotW=Math.max(1,right-left);
  ...
  drawRangeTags(ctx,candles,xs,toY,right,bottom);
  drawTimeAxis(ctx,key,candles,xs,left,right,bottom,h);

  // v15.98.61 — compact live Price Tick: same width as the final candle body.
