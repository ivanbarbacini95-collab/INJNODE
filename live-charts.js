...
// v15.98.75 — Exchange-style order-book aggregation + deep scroll + live trade price flash + instant Home return through browser history/BFCache.
...
const state = {
  ...
  book:{asks:[],bids:[],bestAsk:0,bestBid:0,spread:0,renderAt:0,timer:0,open:false,lastUpdateAt:0,endpointIndex:0,watchdog:0,snapshotTimer:0,snapshotLoading:false,maxLevels:100,rawLimit:100,grouping:savedBookGrouping,askPinned:true,bidPinned:true,priceFlashTimer:0}
};
...
function connect(){
  ...
  try{
    ...
    ws.onmessage=(e)=>{try{const d=JSON.parse(e.data);const price=+d.p,time=+(d.T||d.E||Date.now());if(price>0){applyTrade(price,time);flashBookTrade(price,d.m?'sell':'buy');}}catch(_){}};
    ws.onerror=()=>{};
...
function returnHomeInstant(){
  ...
  location.replace('./index.html?v=15.98.75');
}
$('liveChartsBack')?.addEventListener('click',(e)=>{e.preventDefault();e.stopPropagation();returnHomeInstant();});
document.addEventListener('visibilitychange',()=>{if(document.hidden){if(state.raf)cancelAnimationFrame(state.raf);state.raf=0;try{state.ws?.close();}catch(_){}try{state.depthWs?.close();}catch(_){}}else if(!state.stopped){start();connect();connectBook(state.book.endpointIndex);void loadOrderBookSnapshot();startBookWatchdog();startBookSnapshotRefresh();}});
...
updateBookGroupingUI();
start();connect();void loadOrderBookSnapshot();connectBook(0);startBookWatchdog();startBookSnapshotRefresh();void primeHistories();
