...
// v15.98.63 — Live Charts 2.0 realtime candles: aggTrade drives the active candle trade-by-trade
...
function applyTrade(price,time=Date.now()){if(!(price>0))return;state.lastPrice=price;state.targetPrice=price;if(!(state.visualPrice>0))state.visualPrice=price;state.lastTickAt=Date.now();state.flashUntil=performance.now()+420;for(const key of Object.keys(ranges))mergeTradeInto(key,price,time);}
