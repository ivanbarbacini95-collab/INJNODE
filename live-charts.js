...
// v15.98.65 — Live Charts: denser proportional candles + continuous small-timeframe horizontal flow
// across every timeframe in parallel; all timeframe histories are primed in background so switching
// ranges never lands on a stale candle. Smooth visual interpolation remains display-only.
...
function drawActive(now,dt,ts){
  ...
  // Keep a stable number of horizontal slots so adding a new bucket never changes every candle's spacing.
  const rawCandles=visible.length>cfg.expected?visible.slice(-cfg.expected):visible;
  ...
  const count=candles.length,slotCount=Math.max(cfg.expected,count,1),step=plotW/slotCount;
  // 1/5/10 minute charts advance by a fractional bucket every frame. At the next bucket boundary
  // the previous candle is already exactly one slot to the left, so there is no visible jump.
  const smoothFlow=key==='m1'||key==='m5'||key==='m10';
  const bucketPhase=smoothFlow?Math.max(0,(now-(candles.at(-1)?.t||now))/cfg.bucketMs):0;
  const xStart=right-step*(count-.5+bucketPhase);
  const bodyW=Math.max(1.8,Math.min(48,step*(smoothFlow?.80:.76))),xs=[];
  for(let i=0;i<count;i++){const c=candles[i],x=xStart+step*i;xs.push(x);const up=c.c>=c.o,color=up?state.colors.up:state.colors.down,yH=toY(c.h),yL=toY(c.l),yO=toY(c.o),yC=toY(c.c),isLast=i===count-1;if(x<left-bodyW||x>right+bodyW)continue;ctx.save();ctx.strokeStyle=hexAlpha(color,isLast?.98:.84);ctx.lineWidth=isLast?1.25:1;ctx.beginPath();ctx.moveTo(x,yH);ctx.lineTo(x,yL);ctx.stroke();let by=Math.min(yO,yC),bh=Math.abs(yC-yO);if(bh<1.55){by=(yO+yC)/2-.775;bh=1.55;}ctx.fillStyle=hexAlpha(color,isLast?.98:.90);ctx.fillRect(x-bodyW/2,by,bodyW,bh);if(isLast){ctx.strokeStyle=hexAlpha(color,.98);ctx.lineWidth=.7;ctx.strokeRect(x-bodyW/2-.3,by-.3,bodyW+.6,bh+.6);}ctx.restore();}
  ...
  const last=candles.at(-1),lastX=xs.at(-1),yO=toY(last.o),yC=toY(last.c),bullish=last.c>=last.o;
...
start();connect();void loadShortHistory().then(()=>Promise.allSettled([ensureHistory('d1'),ensureHistory('w1')])).then(()=>ensureHistory(state.active));
