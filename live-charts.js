...
const state = {
  series: { short5: [], short10: [], h1: [], d1: [], w1: [] },
  loaded: { short5: false, short10: false, h1: false, d1: false, w1: false },
  loading: {}, scales: {}, scaleTextAt: {},
  lastPrice: 0, lastTickAt: 0, flashUntil: 0,
  colors: null, ws: null, reconnect: 0, stopped: false,
  raf: 0, lastFrame: 0, visible: new Set(['m5','m10','h1'])
};
...
async function loadShortHistory() {
  for (const k of ['m5','m10','h1']) setLoading(k,'sync');
  try {
    let seconds = [];
    ...
    if (seconds.length) {
      ...
      // Graceful fallback: sparse history until realtime 1-second updates fill it.
    ...
    state.loaded.short5 = state.loaded.short10 = state.loaded.h1 = true;
    for (const k of ['m5','m10','h1']) { trimSeries(k); setLoading(k,'done'); }
    const latest = seconds.at(-1)?.c || minutes.at(-1)?.c;
    if (latest > 0) state.lastPrice = latest;
  } catch (_) {
    for (const k of ['m5','m10','h1']) setLoading(k,'live');
  }
}
async function ensureHistory(key) {
  ...
  try {
    ...
    const latest=rows.at(-1)?.c; if(!(state.lastPrice>0)&&latest>0) state.lastPrice=latest;
  ...
  try {
    ...
    ws.onmessage=(e)=>{
      try {
        ...
        if(!(raw.c>0))return;
        state.lastPrice=raw.c; state.lastTickAt=Date.now(); state.flashUntil=performance.now()+560;
...
function drawOne(now,dt,key,cfg,ts){
  ...
  const candles=visibleCandles(now,key,cfg);if(!candles.length)return;
  ...
  ctx.save();ctx.setLineDash([2,3]);ctx.lineWidth=1;ctx.strokeStyle=hexAlpha(tfColor,.28);ctx.beginPath();ctx.moveTo(lastX+bodyW/2+2,lastY);ctx.lineTo(right,lastY);ctx.stroke();ctx.restore();

  const flash=Math.max(0,Math.min(1,(state.flashUntil-ts)/560));
  const halo=5.8+flash*4.5;
  ctx.beginPath();ctx.arc(Math.min(right-2,lastX+Math.max(bodyW/2+4,5)),lastY,halo,0,Math.PI*2);ctx.fillStyle=hexAlpha(tfColor,.055+flash*.09);ctx.fill();
  ctx.beginPath();ctx.arc(Math.min(right-2,lastX+Math.max(bodyW/2+4,5)),lastY,3.1+flash*.9,0,Math.PI*2);ctx.fillStyle=tfColor;ctx.fill();ctx.lineWidth=1.5;ctx.strokeStyle=state.colors.canvas;ctx.stroke();
...
function frame(ts){
  ...
  if(state.lastPrice>0){setText('headerLivePrice',priceText(state.lastPrice));const now=Date.now();for(const key of state.visible){const cfg=ranges[key];if(cfg)drawOne(now,dt,key,cfg,ts);}}
