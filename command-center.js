...
function drawChart(key,now,dt){
  ...
  const slotCount=Math.max(cfg.maxCandles,candleCount,1);
  const stepX=pw/slotCount;
  const smoothFlow=key==='m1'||key==='m5'||key==='m10';
  const bucketPhase=smoothFlow?((now%cfg.bucket)/cfg.bucket):0;
  const xStart=right-stepX*(candleCount-.5+bucketPhase);
  const minBody=smoothFlow?1.8:2.6;
  const bodyW=Math.max(minBody,Math.min(28,stepX*(smoothFlow?.78:.72)));
  ...
  for(let idx=0; idx<candles.length; idx++){
    ...
    ctx.strokeStyle = hexAlpha(bodyColor,.94);
    ctx.lineWidth = Math.max(1, Math.min(1.7,bodyW*0.10));
    ctx.beginPath();
    ctx.moveTo(x, yh);
    ctx.lineTo(x, yl);
    ctx.stroke();
    ...
    const bodyH=Math.max(2.35, bodyBottom-bodyTop);
    // Solid body deliberately covers the wick through the open/close range: no hollow candle / centre line.
    ctx.fillStyle = bodyColor;
    ctx.fillRect(x-bodyW/2, bodyTop, bodyW, bodyH);
    ctx.strokeStyle = hexAlpha(bodyColor,.98);
    ctx.lineWidth = .65;
  ...
  const lastCloseY=candleY(lastCandle.c,min,span,bottom,ph);
...
function frame(ts){state.raf=0;if(state.stopped||document.hidden)return;const dt=Math.max(8,Math.min(120,ts-(state.lastFrame||ts-50)));state.lastFrame=ts;if(state.target>0){if(!(state.visual>0))state.visual=state.target;const delta=state.target-state.visual;state.visual+=delta*(1-Math.exp(-dt/125));if(Math.abs(delta)<.0000005)state.visual=state.target}if(ts-state.lastDraw>33&&state.visual>0){state.lastDraw=ts;const now=Date.now();Object.keys(ranges).forEach(k=>drawChart(k,now,dt));renderMarket()}state.raf=requestAnimationFrame(frame)}
