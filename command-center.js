...
const ranges = {
  m5:{ms:5*60_000, bucket:5_000, canvas:'ccCanvas5m',card:'.cc-chart-5m',ids:['cc5Top','cc5Mid','cc5Bottom','cc5Marker'],minSpan:.00007,maxCandles:60},
  m10:{ms:10*60_000, bucket:10_000, canvas:'ccCanvas10m',card:'.cc-chart-10m',ids:['cc10Top','cc10Mid','cc10Bottom','cc10Marker'],minSpan:.00010,maxCandles:60},
  h1:{ms:HOUR, bucket:60_000, canvas:'ccCanvas1h',card:'.cc-chart-1h',ids:['cc1hTop','cc1hMid','cc1hBottom','cc1hMarker'],minSpan:.00022,maxCandles:60},
  d1:{ms:DAY, bucket:30*60_000, canvas:'ccCanvas1d',card:'.cc-chart-1d',ids:['cc1dTop','cc1dMid','cc1dBottom','cc1dMarker'],minSpan:.0013,maxCandles:48},
  w1:{ms:7*DAY, bucket:4*HOUR, canvas:'ccCanvas1w',card:'.cc-chart-1w',ids:['cc1wTop','cc1wMid','cc1wBottom','cc1wMarker'],minSpan:.0030,maxCandles:42}
};
const state = {
...
function buildCandles(rows,start,bucketMs,now,lastPrice,maxCandles){
  ...
  for(const row of rows){
    if(!(row?.p>0)) continue;
    const normalized=Math.max(start,row.t);
    const offset=Math.max(0, normalized-start);
    const currentStart=start + Math.floor(offset / bucketMs) * bucketMs;
    if(!active || active.t!==currentStart){
      active={t:currentStart,o:row.p,h:row.p,l:row.p,c:row.p};
      candles.push(active);
    }else{
      active.h=Math.max(active.h,row.p);
      active.l=Math.min(active.l,row.p);
      active.c=row.p;
    }
  }
  if(!candles.length){
    candles.push({t:start,o:lastPrice,h:lastPrice,l:lastPrice,c:lastPrice});
  }
  const finalStart=start + Math.floor(Math.max(0, now-start) / bucketMs) * bucketMs;
  let last=candles[candles.length-1];
  if(!last || last.t!==finalStart){
    const seed=last ? last.c : lastPrice;
    last={t:finalStart,o:seed,h:Math.max(seed,lastPrice),l:Math.min(seed,lastPrice),c:lastPrice};
    candles.push(last);
  }else{
...
function drawChart(key,now,dt){
  ...
  let i=firstAtOrBefore(src,start),rows=src.slice(i).filter(r=>r.t<=now&&r.p>0);
  if(!rows.length)rows=[{t:start,p:state.visual}];
  if(rows[0].t>start)rows.unshift({t:start,p:rows[0].p});
  rows.push({t:now,p:state.visual});
  ...
  ctx.beginPath();
  ctx.arc(lastX,lastY,7.2+pulse*1.5,0,Math.PI*2);
  ctx.fillStyle=hexAlpha(markerColor,.12+pulse*.05);
  ctx.fill();
  ctx.beginPath();
  ctx.arc(lastX,lastY,4.1,0,Math.PI*2);
  ctx.fillStyle=markerColor;
  ctx.fill();
  ctx.lineWidth=1.5;
  ctx.strokeStyle=getComputedStyle(document.documentElement).getPropertyValue('--canvas').trim();
  ctx.stroke();
  ctx.strokeStyle=hexAlpha(markerColor,.32);
  ctx.lineWidth=1;
  ctx.setLineDash([4,4]);
  ctx.beginPath();
