...
// v15.98.55 — Command Center: true edge Price Tick; no connector line across charts.
// v15.98.55 — Full package: Price Tick, full live derived data and per-timeframe percentage badges.
// v15.98.55 — Price Tick + full live derived data updates.
...
function drawChart(key,now,dt){
  ...
  // v15.98.55 — true Price Tick: a compact edge notch at the exact live price.
  // No connector line, no dot, no trail across the chart.
  const tickFresh=Math.max(0,1-(now-state.lastMarketTick)/620);
  ...
  const tickY=candleY(state.visual,min,span,bottom,ph);
  const tickRight=right;
  const tickLeft=tickRight-(11+tickFresh*3);
  const capH=5.5+tickFresh*1.5;
  ctx.save();
  ctx.lineCap='round';
  ctx.lineJoin='round';
  ctx.shadowColor=hexAlpha(tickColor,.22+tickFresh*.26);
  ctx.shadowBlur=2+tickFresh*5;
  ctx.strokeStyle=tickColor;
  ctx.lineWidth=2.15+tickFresh*.55;
  ctx.beginPath();
  ctx.moveTo(tickLeft,tickY);
  ctx.lineTo(tickRight,tickY);
  ctx.stroke();
  ctx.shadowBlur=0;
  ctx.strokeStyle=hexAlpha(tickColor,.78);
  ctx.lineWidth=1.15;
  ctx.beginPath();
  ctx.moveTo(tickLeft,tickY-capH);
  ctx.lineTo(tickLeft,tickY+capH);
  ctx.stroke();
  ctx.restore();
