...
// v15.98.60 — adds 1 MIN while preserving the dominant 1H chart and yellow candle-width Price Tick.
// v15.98.60 — Command Center: true edge Price Tick; no connector line across charts.
// v15.98.60 — Full package: Price Tick, full live derived data and per-timeframe percentage badges.
// v15.98.60 — Price Tick + full live derived data updates.
...
function drawChart(key,now,dt){
  ...
  const minBody=(key==='m1'||key==='m5'||key==='m10')?1.6:3.0;
  const bodyW=Math.max(minBody,Math.min(16,stepX*0.58));
  ...
  // v15.98.60 — yellow candle-width live Price Tick, shared with Live Charts.
  // Bullish final candle -> tick rests just above its body; bearish -> just below.
  // It flashes only when a new market datum arrives.
  const lastCandle=candles.at(-1);
  const lastOpenY=candleY(lastCandle.o,min,span,bottom,ph);
  const lastCloseY=candleY(lastCandle.c,min,span,bottom,ph);
  const lastBodyTop=Math.min(lastOpenY,lastCloseY);
  const lastBodyBottom=Math.max(lastOpenY,lastCloseY);
  const lastBullish=lastCandle.c>=lastCandle.o;
  const tickFresh=Math.max(0,Math.min(1,1-(now-state.lastMarketTick)/560));
  const tickGap=1.5;
  const tickY=Math.max(top+1,Math.min(bottom-1,lastBullish?lastBodyTop-tickGap:lastBodyBottom+tickGap));
  const tickHalf=bodyW/2;
  const tickColor='#ffd84d';
  ctx.save();
  ctx.lineCap='round';
  ctx.strokeStyle=hexAlpha(tickColor,.80+tickFresh*.20);
  ctx.lineWidth=1.9+tickFresh*1.2;
  if(tickFresh>0){
    ctx.shadowColor=hexAlpha(tickColor,.34+tickFresh*.42);
    ctx.shadowBlur=2+tickFresh*6;
  }
  ctx.beginPath();
