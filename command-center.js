...
async function loadMarketHistory(){
  try{
    ...
    state.marketOk=true;state.chartEnterAt=performance.now();renderAll();
  }catch(_){
...
function frame(ts){state.raf=0;if(state.stopped||document.hidden)return;const dt=Math.max(8,Math.min(80,ts-(state.lastFrame||ts-16)));state.lastFrame=ts;if(state.target>0){if(!(state.visual>0))state.visual=state.target;const delta=state.target-state.visual;state.visual+=delta*(1-Math.exp(-dt/38));if(Math.abs(delta)<.00000035)state.visual=state.target}if(ts-state.lastDraw>15&&state.visual>0){state.lastDraw=ts;const now=Date.now();Object.keys(ranges).forEach(k=>drawChart(k,now,dt));renderMarket()}state.raf=requestAnimationFrame(frame)}
