...
function renderWorthSparkline() {
  ...
  if(state.worthRange==='d1'){
    ...
    // Before the first 24h is complete, behave like Home INJ Price: the line
    // begins on the left and physically advances across a 24h canvas over time.
    if(archiveSpan<DAY){startT=firstArchiveT;endT=firstArchiveT+DAY;}
    else {startT=now-DAY;endT=now;}
  }else if(state.worthRange==='w1'){
    startT=now-(7*DAY);endT=now;
  }else if(state.worthRange==='m1'){
    startT=currentMonthStart(now);
    const d=new Date(startT);
    endT=new Date(d.getFullYear(),d.getMonth()+1,1,0,0,0,0).getTime();
  }
