...
function renderWorthSparkline() {
  ...
  // Accurate market-style plot: the vertical scale is linear, padded around the
  // real min/max so small movements remain readable without distorting ratios.
  const reduced = downsampleExtrema(rawSamples, window.innerWidth <= 760 ? 110 : 170);
  const samples = readableWorthSamples(reduced, window.innerWidth <= 760 ? 110 : 170);
  const values = samples.map((row)=>number(row.value));
  const visual = linearVisualPositions(values,.0015);
  const times = samples.map((row) => number(row.t));
  const minT = Math.min(...times), maxT = Math.max(...times), timeSpan = Math.max(1,maxT-minT);
  const plotted = samples.map((row,index) => ({
    t:number(row.t),
  ...
  const first = plotted[0], last = plotted.at(-1);
  const area = `${d} L${last.x.toFixed(2)} 92 L${first.x.toFixed(2)} 92 Z`;
  svg.innerHTML = `${treasuryGridSvg()}<path class="treasury-worth-area" d="${area}"></path><path class="treasury-worth-line" d="${d}"></path>`;
