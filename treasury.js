...
function render() {
  ...
  if (!data) { renderSparkline(); return; }
...
async function loadTreasury(force=false) {
  ...
  try {
    ...
    state.data=data;state.updatedAt=data.updatedAt;persistSnapshot(data);recordHistory(data.total,data.updatedAt);badge.classList.remove('sync','error');
  } catch(error) {
...
async function loadPrice() {
  try { const ticker=await fetchJson('https://api.binance.com/api/v3/ticker/price?symbol=INJUSDT',6000);const price=number(ticker?.price);if(price>0){state.price=price;render();} }
  catch(_) { try { const data=await fetchJson('https://api.coingecko.com/api/v3/simple/price?ids=injective-protocol&vs_currencies=usd',6000);const price=number(data?.['injective-protocol']?.usd);if(price>0){state.price=price;render();} } catch(_){} }
}
function connectPriceSocket() {
  ...
  try {
    ...
    socket.addEventListener('message',(event)=>{try{const tick=JSON.parse(event.data);const price=number(tick?.c);if(price>0){state.price=price;render();}}catch(_){}});
...
function boot() {
  ...
  hydrateSnapshot();hydrateHistory();bindSparkInteraction();render();
