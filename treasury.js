...
function pieSlicePath(startAngle,endAngle){
  ...
  if(span<=0)return '';
  if(span>=359.999)return 'M60 60 m0 -54 a54 54 0 1 1 0 108 a54 54 0 1 1 0 -108';
  const start=piePoint(60,60,54,startAngle),end=piePoint(60,60,54,endAngle);
  const large=span>180?1:0;
  return `M60 60 L${start.x.toFixed(3)} ${start.y.toFixed(3)} A54 54 0 ${large} 1 ${end.x.toFixed(3)} ${end.y.toFixed(3)} Z`;
}
function renderAllocationPie(staked,available,rewards,total){
  ...
  const values=[Math.max(0,number(staked)),Math.max(0,number(available)),Math.max(0,number(rewards))];
  const classes=['staked','available','rewards'];
  let angle=0;
  const paths=[];
  values.forEach((value,index)=>{
    const next=index===values.length-1?360:angle+(value/safe)*360;
    if(next>angle)paths.push(`<path class="treasury-pie-slice ${classes[index]}" d="${pieSlicePath(angle,next)}"></path>`);
    angle=next;
  });
...
function renderWorthSparkline() {
  ...
  const maxPoints = window.innerWidth <= 760 ? 88 : 150;
  ...
  // A minimum 1.5% vertical window keeps tiny realtime price noise from filling
  // the whole panel, while larger real moves still use the available height.
  const stableSpan = Math.max(rawSpan,Math.abs(lastValue)*.015,1);
  const center = rawSpan < stableSpan ? (minValue+maxValue)/2 : (minValue+maxValue)/2;
  const paddedSpan = stableSpan * 1.24;
  const displayMin = center - paddedSpan/2;
  const displayMax = center + paddedSpan/2;
  const valueSpan = Math.max(1e-9,displayMax-displayMin);
  const times = samples.map((row)=>number(row.t));
  const minT = Math.min(...times), maxT = Math.max(...times), timeSpan = Math.max(1,maxT-minT);
  const topY = 12, bottomY = 84, chartHeight = bottomY-topY;
  ...
  const d = worthSmoothPath(plotted);
  const first = plotted[0], last = plotted.at(-1);
  const area = `${d} L${last.x.toFixed(2)} 92 L${first.x.toFixed(2)} 92 Z`;
  const liveY = Math.max(topY,Math.min(bottomY,bottomY-((last.value-displayMin)/valueSpan)*chartHeight));
  const grid = [12,48,84].map((y)=>`<line class="treasury-chart-gridline" x1="0" y1="${y}" x2="320" y2="${y}"></line>`).join('');
...
function bindWorthInteraction() {
  ...
  const release = (id) => { try{if(stage.hasPointerCapture?.(id))stage.releasePointerCapture(id);}catch(_){} };
  const clearMouseIdle = () => { if(mouseIdleTimer){clearTimeout(mouseIdleTimer);mouseIdleTimer=0;} };
  const armMouseIdle = () => { clearMouseIdle(); mouseIdleTimer=setTimeout(()=>{mouseIdleTimer=0;hideWorthInspector();},950); };
  const leaveLive = () => { clearMouseIdle(); hideWorthInspector(); };
  stage.addEventListener('pointerdown',(event)=>{
    if(event.pointerType==='mouse'){inspectWorthAt(event);armMouseIdle();return;}
    clearMouseIdle();touch.id=event.pointerId;touch.x=event.clientX;touch.y=event.clientY;touch.horizontal=false;inspectWorthAt(event);
  },{passive:true});
  stage.addEventListener('pointermove',(event)=>{
    if(event.pointerType==='mouse'){
      if(!inside(event)){leaveLive();return;}
      inspectWorthAt(event);armMouseIdle();return;
    }
    if(event.pointerId!==touch.id)return;
    if(!inside(event)){release(event.pointerId);stage.classList.remove('dragging');touch.id=null;touch.horizontal=false;leaveLive();return;}
    const dx=event.clientX-touch.x,dy=event.clientY-touch.y,ax=Math.abs(dx),ay=Math.abs(dy);
  ...
  document.addEventListener('visibilitychange',()=>{if(document.hidden)leaveLive();});
  document.addEventListener('pointerdown',(event)=>{if(stage.classList.contains('inspecting')&&!stage.contains(event.target))leaveLive();},true);
  stage.addEventListener('keydown',(event)=>{const samples=state.worthPlot?.samples||[];if(!samples.length||!['ArrowLeft','ArrowRight','Home','End','Escape'].includes(event.key))return;event.preventDefault();if(event.key==='Escape')return leaveLive();let i=state.worthInspectIndex;if(event.key==='Home')i=0;else if(event.key==='End')i=samples.length-1;else if(event.key==='ArrowLeft')i=i<0?samples.length-1:i-1;else i=i<0?0:i+1;showWorthSample(i);});
}
function render() {
...
function installTreasuryMobileScrollGuard(){
  const scroller=document.querySelector('.treasury-shell');
  if(!scroller||scroller.dataset.mobileEdgeGuard==='1')return;
  scroller.dataset.mobileEdgeGuard='1';
  let tracking=false,lastY=0,startX=0,startY=0;
  const mobileLike=()=>window.matchMedia('(max-width: 760px)').matches||window.matchMedia('(pointer: coarse)').matches;
  const maxScroll=()=>Math.max(0,scroller.scrollHeight-scroller.clientHeight);
  scroller.addEventListener('touchstart',(event)=>{
    if(!mobileLike()||event.touches.length!==1){tracking=false;return;}
    const touch=event.touches[0];startX=touch.clientX;startY=touch.clientY;lastY=touch.clientY;
    const max=maxScroll();
    if(max>2){if(scroller.scrollTop<=0)scroller.scrollTop=1;else if(scroller.scrollTop>=max)scroller.scrollTop=max-1;}
    tracking=true;
  },{passive:true});
  scroller.addEventListener('touchmove',(event)=>{
    if(!tracking||!mobileLike()||event.touches.length!==1)return;
    const touch=event.touches[0],dx=touch.clientX-startX,dy=touch.clientY-startY,stepY=touch.clientY-lastY;lastY=touch.clientY;
    if(Math.abs(dx)>Math.abs(dy)||Math.abs(stepY)<.2)return;
    const max=maxScroll(),atTop=scroller.scrollTop<=1.25,atBottom=scroller.scrollTop>=max-1.25;
...
function boot() {
  ...
  hydrateSnapshot();hydrateHistory();hydrateWorthHistory();bindTreasuryHomeReturn();bindSparkInteraction();bindWorthInteraction();bindWorthRangeControls();installTreasuryMobileScrollGuard();render();
  void restoreTimelineBackups().then(()=>render());
  ...
  document.addEventListener('visibilitychange',()=>{
    ...
    else{void restoreTimelineBackups().then(()=>render());void loadPrice();void loadTreasury(true);connectPriceSocket();}
  });
  window.addEventListener('pageshow',()=>{try{state.currency=localStorage.getItem('inj_monitor_currency')==='EUR'?'EUR':'USD';}catch(_){};void restoreTimelineBackups().then(()=>render());render();});
