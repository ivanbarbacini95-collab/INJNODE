...
function renderWorthSparkline() {
  ...
  if (!svg) return;
  let samples = state.worthSamples.filter((row) => number(row.t) > 0 && number(row.value) > 0);
  const liveWorth = number(state.data?.total) * number(state.price);
  if (liveWorth > 0) {
    const last = samples.at(-1);
    if (!last || Math.abs(number(last.value) - liveWorth) > Math.max(.000001,Math.abs(liveWorth)*1e-9)) samples = [...samples,{t:Date.now(),value:liveWorth}];
  }
  if (samples.length < 2) {
    state.worthPlot = null;
    svg.innerHTML = '<path class="treasury-worth-line" d="M0 76 L320 76" opacity=".18"></path>';
    $('treasuryWorthHistoryStart').textContent = samples.length ? historyLabel(samples[0].t) : 'Prima lettura';
  ...
  $('treasuryWorthHistoryStart').textContent = historyLabel(samples[0].t);
  $('treasuryWorthHistoryEnd').textContent = historyLabel(samples.at(-1).t);
...
function boot() {
  ...
  hydrateSnapshot();hydrateHistory();hydrateWorthHistory();bindSparkInteraction();bindWorthInteraction();render();
