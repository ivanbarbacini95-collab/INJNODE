...
function renderWorthSparkline() {
  ...
  // v15.99.59 — same plot behaviour as the Home INJ Price chart:
  // true min/max drive the scale, 20% breathing room, linear path and a fixed
  // time window for 24H/1W/1M so the curve never re-stretches unexpectedly.
  ...
  const selectedWindow = WORTH_RANGES[state.worthRange] || DAY;
  const minSampleT = Math.min(...samples.map((row)=>number(row.t)));
  const maxSampleT = Math.max(...samples.map((row)=>number(row.t)));
  const startT = state.worthRange === 'all' || !Number.isFinite(selectedWindow)
    ? minSampleT
    : Math.min(maxSampleT,liveEnd-selectedWindow);
  const endT = state.worthRange === 'all' || !Number.isFinite(selectedWindow)
    ? Math.max(maxSampleT,liveEnd)
    : liveEnd;
